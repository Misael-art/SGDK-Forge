...
int main()
{
  ...
  PAL_setColor(14, colour_red);
  PAL_setColor(13, colour_green);
  PAL_setColor(12, colour_blue);
  PAL_setColor(11, colour_yellow);
  ...
  while(TRUE)
  {
      ...
      for (i = 1; i < 255; i ++)
      {
        ...
        if (cosFix16(i*8) != 0){
          BMP_setPixel(i, 80 + (F16_div(sinFix16(i*8), cosFix16(i*8))), pal_yellow);
        }
        BMP_flip(FALSE);
      }
