...
if (isTransitioning) {
	switch(transitionDirection) {
		case 0: // Right
			...
			if (F32_toInt(transitionOffset) >= HORIZONTAL_RESOLUTION) {
			...
			VDP_setHorizontalScroll(BG_A, -F32_toInt(transitionOffset));
			VDP_setHorizontalScroll(BG_B, -F32_toInt(transitionOffset));
			break;
		...
		case 2: // Down
			transitionOffset += SCROLL_SPEED;
			if (F32_toInt(transitionOffset) >= VERTICAL_RESOLUTION) {
				VDP_clearPlane(BG_A, TRUE);
				VDP_clearPlane(BG_B, TRUE);
				// updateCurrentRoom();
				// convertMapArrays();
			...
			VDP_setVerticalScroll(BG_A, F32_toInt(transitionOffset));
			VDP_setVerticalScroll(BG_B, F32_toInt(transitionOffset));
			break;
	...
	SPR_setPosition(player, F32_toInt(playerPosX) - current_cam_x, F32_toInt(playerPosY) - current_cam_y);
	return;
}
	...
	int player_x_map_int = F32_toInt(playerPosX);
	int player_y_map_int = F32_toInt(playerPosY);
	...
	SPR_setPosition(player, F32_toInt(playerPosX) - new_cam_x, F32_toInt(playerPosY) - new_cam_y);
}
