...
void collision(){
	...
	s16 player_coll_coord_up = F32_toInt(playerPosY) + PLAYER_COL_UP;
	s16 player_coll_coord_down = F32_toInt(playerPosY) + PLAYER_COL_DOWN;
	...
	s16 playerX = F32_toInt(playerPosX);
	s16 playerY = F32_toInt(playerPosY);
	s16 merchantX = F32_toInt(merchantPosX); 
	s16 merchantY = F32_toInt(merchantPosY); 
	...
	if(player_move_left){
		playerPosX -= playerSpeed;
		player_coll_coord_left = F32_toInt(playerPosX) + PLAYER_COL_LEFT;
		...
		if((tile_type_coll_topleft == SOLID_TILE)||(tile_type_coll_bottomleft == SOLID_TILE)){
			...
			playerPosX = FIX32(blocked_coord);
		}
		...
		if((tile_type_coll_topleft == EXIT_TILE)||(tile_type_coll_bottomleft == EXIT_TILE)){
			//VDP_clearTileMapRect(BG_B, (F32_toInt(playerPosX) >> 3),(F32_toInt(playerPosY) >> 3),2,2);
		...
		playerPosX += playerSpeed;
		player_coll_coord_right = F32_toInt(playerPosX) + PLAYER_COL_RIGHT;
	...
	if (player_move_up){
		playerPosY -= playerSpeed;
		player_coll_coord_up = F32_toInt(playerPosY) + PLAYER_COL_UP;
		...
		if((tile_type_coll_topright == SOLID_TILE)||(tile_type_coll_topleft == SOLID_TILE)){
			...
			playerPosY = FIX32(blocked_coord);
		}
		...
		playerPosY += playerSpeed;
		player_coll_coord_down = F32_toInt(playerPosY) + PLAYER_COL_DOWN;
