        ...
        for (yy = 0; yy < mh; yy += 2) {
            ...
            if (LEVEL_COL2[mapIndex] == 10) {
                ...
                if(MapHasBeenMade == FALSE){
                    playerPosX = FIX32(xx) << 3;
                    playerPosY = FIX32(yy) << 3;
...
void showMerchant(){
        ...
        merchant = SPR_addSprite(&merchantSprite, F32_toInt(merchantPosX), F32_toInt(merchantPosY), TILE_ATTR(PAL3, FALSE, FALSE, FALSE));
        SPR_setPosition(merchant, F32_toInt(merchantPosX), F32_toInt(merchantPosY));
