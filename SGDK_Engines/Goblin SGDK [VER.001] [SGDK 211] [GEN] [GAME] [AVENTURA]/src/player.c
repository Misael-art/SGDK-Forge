...
void displayPlayer(){
	...
	player = SPR_addSprite(&our_sprite, F32_toInt(playerPosX), F32_toInt(playerPosY), TILE_ATTR(PAL2, FALSE, FALSE, FALSE));
...
void screenWarble() {
    ...
    for (int i = 0; i < 224; i++) {
        offset[i] = F16_toInt(FIX16(.04) * sinFix16(warblePhase + FIX16(i * 3)));
    }
