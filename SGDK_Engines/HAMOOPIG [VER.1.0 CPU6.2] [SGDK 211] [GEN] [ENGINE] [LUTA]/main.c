...
// Esta variante expande a base HAMOOPIG com campanha, IA e pausa.
// Para estudar o arquivo, vale separar mentalmente em tres blocos:
// fluxo de telas via gRoom, nucleo da luta na Room 10 e configuracoes extras de campanha/IA.
...
//--- VARIAVEIS ---//
// Como a campanha e a IA dividem o mesmo arquivo com a luta,
// quase tudo fica em estado global compartilhado entre rooms e subsistemas.
...
// O main configura video, prepara campanha/IA e depois entra no despachante por rooms.
// A maior parte da logica de jogo continua acontecendo dentro das funcoes chamadas por cada room.
int main(u16 hard) /************** MAIN **************/
{
	...
	// Estas tabelas ajustam o comportamento da IA e a progressao da campanha.
	// Ler este bloco primeiro ajuda a entender por que cada fase reage de um jeito.
	//DIFICULDADE DA IA
    ...
    {
		// gRoom segue sendo a maquina de telas principal:
		// abertura, menus, campanha, luta e pos-luta.
        gFrames++;
		...
		if(gRoom==3) //CHAR SELECT ----------------------------------------------------------------
		{
			...
			*/
			...
			)
			{
				...
				//GE[7].sprite = SPR_addSpriteExSafe(&spr_color_cursor, 69, 105, TILE_ATTR(PAL0, FALSE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | SPR_FLAG_AUTO_VRAM_ALLOC);
			...
			if(cursorP1ConfirmFX<cursorConfirmTimer)
			{
				if(cursorP1ConfirmFX>0)
				{
					...
					)
					{
						...
						//if (IAP2) GE[7].sprite = SPR_addSpriteExSafe(&spr_color_cursor_static,  69, 105, TILE_ATTR(PAL0, FALSE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | SPR_FLAG_AUTO_VRAM_ALLOC);
                        //else GE[7].sprite = SPR_addSpriteExSafe(&spr_color_cursor_static,  69+8, 105, TILE_ATTR(PAL0, FALSE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | SPR_FLAG_AUTO_VRAM_ALLOC);
					}
				}
			}
		...
		// Room 10 continua sendo o nucleo da luta.
		// Nesta variante, a pausa pode congelar parte do pipeline sem sair da room.
		if(gRoom==10) //IN GAME -------------------------------------------------------------------
		{
