...
NOINLINE void displayBarbPictureFX(void)
{
	...
	/* Draw the background */
	VDP_drawImageEx(BG_B, &barb_pic_2_back, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 0, 0, FALSE, TRUE);
	vramIndex += barb_pic_2_back.tileset->numTile;

	/* Draw the foreground */
	VDP_drawImageEx(BG_A, &barb_pic_2_front, TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, vramIndex), 0, 0, FALSE, TRUE);
	...
	while(fx_phase < fx_len)
	{
		...
		switch(f_step)
		{
			...
			case 3:
				...
				{
					// KLog_U1("-> f_step = ", f_step);
					PAL_fade(16, 31, palette_white, barb_pic_2_front.palette->data, RSE_FRAMES(16), TRUE);
					PAL_interruptFade();
					fading = 1;
					f_step++;		
				}
			...
			case 4:
				...
				{
					// KLog_U1("-> f_step = ", f_step);
					PAL_fade(0, 15, palette_black, barb_pic_2_back.palette->data, RSE_FRAMES(16), TRUE);
					PAL_interruptFade();
					fading = 1;						
					f_step++;
				}
		...
		VDP_setVerticalScroll(BG_B, scroll_A_y);
		fx_phase++;
	}
	...
	for (i = 0; i < 32 ; ++i)
	{
		pal_sort[i].index = i;
		pal_sort[i].value =
				F16_mul(FIX16(0.299), FIX16(pal[i] & 0xf)) +
				F16_mul(FIX16(0.587), FIX16((pal[i] >> 4) & 0xf)) +
				F16_mul(FIX16(0.114), FIX16((pal[i] >> 8) & 0xf));
	}
