...
NOINLINE Sprite **displayBarbTitleFX(void)
{
	...
	/* Draw the foreground */
	VDP_drawImageEx(BG_A, &barb_pic_front, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 0, 0, FALSE, TRUE);
	vramIndex += barb_pic_front.tileset->numTile;

	/* Draw the background */
	VDP_drawImageEx(BG_B, &barb_pic_back_0, TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, vramIndex), 0, 0, FALSE, TRUE);
	vramIndex += barb_pic_back_0.tileset->numTile;	

	VDP_drawImageEx(BG_B, &barb_pic_back_1, TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, vramIndex), 0, 64 >> 3, FALSE, TRUE);
	vramIndex += barb_pic_back_1.tileset->numTile;

	VDP_drawImageEx(BG_B, &barb_pic_back_2, TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, vramIndex), 0, 128 >> 3, FALSE, TRUE);
	vramIndex += barb_pic_back_2.tileset->numTile;

	VDP_drawImageEx(BG_B, &barb_pic_back_3, TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, vramIndex), 0, (128 + 80) >> 3, FALSE, TRUE);
	...
	VDP_setVerticalScroll(BG_A, 280);
...
u8 music_checkpoint = 0;
	...
	PAL_fade(0, 63, palsrc, paldst, RSE_FRAMES(32), TRUE);
