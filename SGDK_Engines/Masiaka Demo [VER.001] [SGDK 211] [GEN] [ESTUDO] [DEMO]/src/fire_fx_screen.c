...
NOINLINE void fireFX(void)
{
	...
	VDP_drawImageEx(BG_A, &conan_on_fire, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, vramIndex), 1, 0, FALSE, TRUE);
	...
	VDP_setVerticalScrollTile(BG_A, 0, scroll, FB_MAX_WIDTH / 16, FALSE);

	firefx_init(BG_B, PAL1);

	DMA_flushQueue();
	VDP_setEnable(1);

	PAL_fadeAll(palette_black, firePal, fadeInTime, TRUE);
	...
	while(music_sync < 4705)
	{
		...
		if (vcount == SCROLL_LENGTH)
			PAL_fadePalette(PAL2, &firePal[32], pentacle.palette->data, RSE_FRAMES(32), TRUE);
	...
	while(i < RSE_FRAMES(64))
	{
		...
		if (i == RSE_FRAMES(64 - 16))
			PAL_fadeOut(32, 32 + 15, RSE_FRAMES(16), TRUE);
		// firefx is 30fps...
		VDP_waitVSync();
