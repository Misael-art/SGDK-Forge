...
static void computeWaveScale(s16 alpha) {
	for (s16 i = 0; i < FIRE_COMPUTED_WIDTH / 8; ++i) {
		fix16 ratio = ((F16_mul(sinFix16((alpha << 5) + (i << 7))) + (FIX16(0.33))), FIX16(0.66));
		fix16 acc = ((FIX16(-SCREEN_HEIGHT + FIRE_HEIGHT)) + (F16_mul(ratio, FIX16(FIRE_DIST_HEIGHT / FIRE_DIST_STEP))));
		for (s16 j = 0; j < FIRE_DIST_HEIGHT / FIRE_DIST_STEP; ++j) {
			waveScale[j][i] = F16_toInt(acc);
			acc = ((acc) - (ratio));
		}
	}
}
...
#if 1
	// hardcoded for BG_B and FIRE_COMPUTED_WIDTH = 96px
