...
NOINLINE void flamesWavesFX(void)
{
	...
	/* Load the fond tiles */
	VDP_drawImageEx(BG_A, &flames_0, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 0, 0, FALSE, FALSE);

	VDP_drawImageEx(BG_A, &flames_0, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 0, 256 >> 3, FALSE, FALSE);
	vramIndex += flames_0.tileset->numTile;;

	VDP_drawImageEx(BG_A, &flames_1, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 10, 0, FALSE, FALSE);

	VDP_drawImageEx(BG_A, &flames_1, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 10, 256 >> 3, FALSE, FALSE);
	vramIndex += flames_1.tileset->numTile;

	VDP_drawImageEx(BG_A, &flames_2, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 20, 0, FALSE, FALSE);

	VDP_drawImageEx(BG_A, &flames_2, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 20, 256 >> 3, FALSE, FALSE);
	vramIndex += flames_2.tileset->numTile;

	VDP_drawImageEx(BG_A, &flames_3, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 30, 0, FALSE, FALSE);

	VDP_drawImageEx(BG_A, &flames_3, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 30, 256 >> 3, FALSE, FALSE);
	vramIndex += flames_3.tileset->numTile;

	VDP_setVerticalScroll(BG_B, (easing_table[64 << 4] >> 4) - 64 - 64);
	VDP_drawImageEx(BG_B, &masiaka_title_pic, TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, vramIndex), (320 - 240) >> 4, (224 - 48) >> 4, FALSE, TRUE);
	...
	sprites[0] = SPR_addSpriteEx(&sword, (320 - 48) >> 1, -128, TILE_ATTR_FULL(PAL3, FALSE, FALSE, FALSE, vramIndex), 0, SPR_FLAG_AUTO_VISIBILITY | !SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
    ...
    while (vcount < 60 * 10)
    {
        ...
        {
        	case 0:
	        	PAL_fadeTo(16, 31, palette_white, 6, TRUE);
	        	break;        	
        	case 2 + 8:
