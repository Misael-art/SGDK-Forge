...
NOINLINE void flat3DGlenzFX(u16 in_phase)
{
	...
	PAL_setPalette(PAL3, palette_white, DMA);
	...
	// KLog_S4("x_glenz_f32 = ", F32_toInt(x_glenz_f32), "y_glenz_f32 = ", F32_toInt(y_glenz_f32), "velx = ", F32_toInt(velx), "vely = ", F32_toInt(vely));
	...
	while(music_checkpoint < 3)
	{
		...
		if (framerate == 60)
		{
			...
			x_glenz_f32 = fix32Add(x_glenz_f32, F32_mul(TIME_SCALE, velx));
			y_glenz_f32 = fix32Add(y_glenz_f32, F32_mul(TIME_SCALE, vely));
		}
		else
		{
			vely = fix32Add(vely, GRAVITY_VELY_PAL);
			x_glenz_f32 = fix32Add(x_glenz_f32, F32_mul(TIME_SCALE_PAL, velx));
			y_glenz_f32 = fix32Add(y_glenz_f32, F32_mul(TIME_SCALE_PAL, vely));
		}

		/* Bouncing */
		if (x_glenz_f32 > FIX32(320 - BALL_RADIUS * 0.85))
		{
			velx = F32_mul(BOUNCE_ENERGY_X, fix32Neg(velx));
		...
		{
			vely = F32_mul(BOUNCE_ENERGY_Y, fix32Neg(vely));
			// vely = fix32Neg(vely);
		...
		x_glenz = F32_toInt(x_glenz_f32) - BALL_RADIUS;
		y_glenz = F32_toInt(y_glenz_f32);
