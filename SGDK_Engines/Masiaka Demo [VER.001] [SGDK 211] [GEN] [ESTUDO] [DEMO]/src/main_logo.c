...
NOINLINE void main_logo(void)
{
	...
	void inline drawPoints(u8 col)
	{
		...
		if (zsort_switch == 0)
		{
			...
			for(i = 0; i < poly_count; i++)
			{
				...
				if (poly_ind[j+3] != -1)
					z_sum = ((((pts_3D[poly_ind[j]].z) + (pts_3D[poly_ind[j+1]].z))) + (((pts_3D[poly_ind[j+2]].z)) + (pts_3D[poly_ind[j+3]].z)));
				else
					z_sum = ((((pts_3D[poly_ind[j]].z) + (pts_3D[poly_ind[j+1]].z))) + (pts_3D[poly_ind[j+2]].z));
		...
		switch(logo_state)
		{
			...
			case 1:
				...
				if (easing_index < EASING_TABLE_LEN_FP)
					rotation.y = F16_mul(FIX16(-4.0), easing_fix16[EASING_TABLE_LEN_FP - easing_index - 1]) >> 4; /* Shift by >> 4 : rotation by 90°, >> 3 : 180°, >> 2 : 360° */
				else
					logo_state = 2;
			...
			case 12:
				...
				{ 		
					VDP_waitVSync();
					PAL_fadeTo(0, 63, palette_white, RSE_FRAMES(32), TRUE);
					logo_state = 13;
				}
				break;
	...
	VDP_drawImageEx(BG_A, &logo_rse_3d, TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, vramIndex), 0, ((240 - 64) >> 4) - 1, FALSE, TRUE);
	...
	VDP_drawImageEx(BG_B, &medieval_girl, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, vramIndex), 0, ((240 - 64) >> 4), FALSE, FALSE);

	VDP_fadePalTo(PAL0, medieval_girl.palette->data, RSE_FRAMES(16), TRUE);
	
	RSE_pause(RSE_FRAMES(120));

	PAL_fadeOut(1, 63, RSE_FRAMES(32), FALSE);
