...
static void hInt(void)
{
	...
	if (lineidx >= ypos[2])
	{
		VDP_setVerticalScroll(BG_B, ypos[1]);
	}
	else if (lineidx >= ypos[3] && lineidx < ypos[2])
	{
		VDP_setVerticalScroll(BG_B, VILLAGE_Y - lineidx);
	}
}
 
static void vInt(void)
{
	VDP_setVerticalScroll(BG_B, ypos[4]);
}
...
NOINLINE void owlFX(void)
{
	...
	VDP_drawImageEx(BG_A, &rm_layer_sky, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 0, 0,  FALSE, TRUE);
	vramIndex += rm_layer_sky.tileset->numTile;
	VDP_drawImageEx(BG_B, &rm_layer_village, TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, vramIndex), 0, VILLAGE_Y / 8,  FALSE, TRUE);
	vramIndex += rm_layer_village.tileset->numTile;
	DMA_flushQueue();
	VDP_drawImageEx(BG_B, &rm_layer_bird, TILE_ATTR_FULL(PAL2, TRUE, FALSE, FALSE, vramIndex), BIRD_X / 8, SYSTILES_Y_OFF / 8,  FALSE, TRUE);
	vramIndex += rm_layer_bird.tileset->numTile;
	DMA_flushQueue();
	
	SPR_init();
	Sprite * trunk_top = SPR_addSpriteEx(&rm_layer_trunk_top, TRUNK_TOP_X, TRUNK_TOP_Y,
			TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, 0xF600 / TILE_SIZE), 0, SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
	SPR_setVisibility(trunk_top, VISIBLE);

	Sprite * trunk_btm = SPR_addSpriteEx(&rm_layer_trunk_btm, TRUNK_BTM_X, TRUNK_BTM_Y,
			TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, vramIndex), 0, SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
	...
	SPR_update();
	VDP_setVerticalScroll(BG_B, (rm_layer_bird.map->h << 3));
	VDP_setVerticalScroll(BG_A, SKY_START_Y);
	...
	VDP_setHorizontalScrollLine(BG_A, 0, hscroll, SCREEN_HEIGHT, TRUE);
	VDP_setHorizontalScrollLine(BG_B, 0, hscroll, SCREEN_HEIGHT, TRUE);
	...
	while(vcount < fxLen)
	{
		...
		if (vcount > initFade && vcount < birdFadeStart)
		{
			...
			PAL_setPalette(PAL2, &pal[32], DMA);
		}
		...
		if (vcount >= initFade && vcount < scrollLen + initFade)
		{
			...
			VDP_setVerticalScroll(BG_A, SKY_START_Y - ypos[1]);
		}
		...
		if (vcount <= sineEnd)
		{
			for (s16 i = 0; i < ypos[3]; i += 2)
			{
				...
				hscroll[i] =  F16_toInt(F16_mul(sinFix16(alpha), mul));
				hscroll[i + 1] = F16_toInt(F16_mul(cosFix16(alpha), mul));
			}
			VDP_setHorizontalScrollLine(BG_B, 0, hscroll, ypos[3], FALSE);
		}
	...
	PAL_fadeTo(0, 47, palette_black, END_FADE_BLACK, FALSE);
