...
u16 RSE_pgwriterDrawString(char *str)
{
	...
	if (c != ' ')
	{
		i = charToTileIndex(c);
		if (i != 0xFF)
			VDP_setTileMapXY(pg_current_plan, TILE_ATTR_FULL(PAL2, TRUE, FALSE, FALSE, TILE_USER_INDEX + i), pg_current_char_x + pg_x_offset, pg_current_char_y);
			// VDP_setTileMapXY(pg_current_plan, TILE_USER_INDEX + i + (FONT_LINE_OFFSET * fade), pg_current_char_x + fade + pg_x_offset, TILE_ATTR_FULL(pg_current_pal, FALSE, FALSE, FALSE, pg_current_char_y));
	}
	else
