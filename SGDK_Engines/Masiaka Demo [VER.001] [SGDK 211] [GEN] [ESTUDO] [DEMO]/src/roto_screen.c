...
static void vInt(void) {
	...
	VDP_setVerticalScroll(BG_B, RZ_Y_OFFSET);
...
NOINLINE void rotoFX(void)
{
	...
	VDP_setPlaneSize(32, 64);
	VDP_drawImageEx(BG_A, &roto_col, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, vramIndex), 1, 0, FALSE, TRUE);
	...
	for(s16 i = 0; i < roto_col.map->h; ++i)
	{
		for(s16 j = 0; j < roto_col.map->w; ++j)
		{
			...
			VDP_setTileMapXY(BG_A, itm, 30 - j, i);
		}
	}

	vramIndex += roto_col.tileset->numTile;

	rzfx_init(BG_B, PAL1, roto_tex.image);
	preparePlan(PAL2);
	VDP_setHorizontalScroll(BG_B, RZ_X_OFFSET);
	VDP_setVerticalScroll(BG_A, PILLAR_Y_OFFSET);
	...
	for(s16 i = 0; i < RZ_TABLES_LEN; ++i)
	{
		...
		angles[i] = RZ_INIT_ANGLE + F16_toRoundedInt(F16_mul(ainc + (easing_fix16[vcountf & (EASING_TABLE_LEN_FP - 1)] << 7), FIX16(0.859)));
