...
NOINLINE void shieldAnimFX(void)
{
	...
	VDP_setPlaneSize(128, 32);
	VDP_setHilightShadow(1); 
	VDP_setScrollingMode(HSCROLL_TILE, VSCROLL_PLANE);

	// PAL_setPalette(PAL0, shield_anim_0.palette->data, DMA);

	// vramIndex = fontIndex;

	/* Draw the background */
	for(i = 0; i < 4; i++)
		VDP_drawImageEx(BG_B, &shield_anim_0, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 0, i << 3 , FALSE, TRUE);

	vramIndex += shield_anim_0.tileset->numTile;

	for(i = 0; i < 4; i++)
		VDP_drawImageEx(BG_B, &shield_anim_1, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), 512 >> 3, i << 3 , FALSE, TRUE);

	vramIndex += shield_anim_1.tileset->numTile;

	for(i = 0; i < 16; i++)
		VDP_drawImageEx(BG_A, &transition_pattern, TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, vramIndex), 0, i << 1, FALSE, FALSE);
	...
	{
		sprites[i >> 1] = SPR_addSpriteEx(&ball_square, -80, -80, TILE_ATTR_FULL(PAL3, FALSE, FALSE, FALSE, tileIndexes[0]), 0, !SPR_FLAG_AUTO_VISIBILITY | !SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_VRAM_ALLOC | !SPR_FLAG_AUTO_TILE_UPLOAD);
	...
	l = l >> 3;
	VDP_setVerticalScroll(BG_A, l - (1024 >> 3));
	...
	while(!should_exit || VDP_isDoingFade())
	{
		...
		if (vcount > RSE_FRAMES(40) && vcount < RSE_FRAMES(80))
		{
			...
			VDP_setHorizontalScrollTile(BG_A, 0, wipe_tile_x, 8 << 2, TRUE);
		}
		else
		...
		if (vcount > RSE_FRAMES(200))
		{
			...
			switch(letter_state)
			{
				...
				case 3:
					...
					{
						case 0:
							PAL_setPalette(PAL3, square_pal_1.palette->data, DMA);
							// PAL_setPalette(PAL0, palette_bg_0, DMA);
							VDP_fadePalTo(PAL0, palette_bg_0, RSE_FRAMES(8), TRUE);
							break;
						case 1:
							PAL_setPalette(PAL3, square_pal_2.palette->data, DMA);
							// PAL_setPalette(PAL0, palette_bg_1, DMA);
							VDP_fadePalTo(PAL0, palette_bg_1, RSE_FRAMES(8), TRUE);
							break;
						case 2:
							PAL_setPalette(PAL3, square_pal_3.palette->data, DMA);
							// PAL_setPalette(PAL0, palette_bg_2, DMA);
							VDP_fadePalTo(PAL0, palette_bg_2, RSE_FRAMES(8), TRUE);
							break;
						case 3:
							PAL_setPalette(PAL3, square_pal_4.palette->data, DMA);
							// PAL_setPalette(PAL0, palette_bg_3, DMA);
		...
		VDP_setHorizontalScrollTile(BG_B, 0, scroll_tile_x, 8 << 2, TRUE);
