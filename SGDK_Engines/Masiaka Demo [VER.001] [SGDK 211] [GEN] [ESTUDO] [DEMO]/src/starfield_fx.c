...
/*	fontIndex = */RSE_pgwriterSetup();
	...
	{
		VDP_drawImageEx(BG_B, &bg_sky, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, vramIndex), loop * 10, (224 - 160) >> 4, FALSE, TRUE);
	}

	vramIndex += bg_sky.tileset->numTile;

	VDP_drawImageEx(BG_A, &rse_retrowave_logo, TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, vramIndex), (320 - 140) >> 4, 4, FALSE, TRUE);
	...
	{
		sprites[loop] = SPR_addSpriteEx(&sprite_stars, 0, 0, TILE_ATTR_FULL(PAL3, FALSE, FALSE, FALSE, tileIndexes[loop & 0x7]), 0, !SPR_FLAG_AUTO_VISIBILITY | !SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_VRAM_ALLOC | !SPR_FLAG_AUTO_TILE_UPLOAD);
		if (!sprites[loop])
		{
	...
	while(TRUE)
	{
		...
		VDP_setVerticalScrollTile(BG_A, 0, (s16 *)&(scroll_v[(((cosFix16(vcount << 3) + 64) * (SCR_SCROLL_STEP_W - 1)) >> 7) * SCR_TILE_W]), SCR_TILE_W, FALSE);
		VDP_setHorizontalScrollTile(BG_A, 0, (s16 *)&(scroll_h[(((cosFix16(vcount << 3) + 64) * (SCR_SCROLL_STEP_H - 1)) >> 7) * SCR_TILE_H]), SCR_TILE_H, FALSE);
