...
void RSE_clearTileRowB(u16 row)
{
	...
	for(col = 0; col < max_col; col++)
		VDP_setTileMapXY(BG_B, TILE_ATTR_FULL(PAL0, 0, 0, 0, TILE_SYSTEMINDEX), col, row);
}
...
void RSE_clearTileRowBWithPrio(u16 row)
{
	...
	max_col = VDP_getPlanWidth(); 
	for(col = 0; col < max_col; col++)
		VDP_setTileMapXY(BG_B, TILE_ATTR_FULL(PAL0, TRUE, 0, 0, TILE_SYSTEMINDEX), col, row);
}
...
void RSE_clearTileRowA(u16 row)
{
	...
	max_col = VDP_getPlanWidth(); 
	for(col = 0; col < max_col; col++)
		VDP_setTileMapXY(BG_A, TILE_ATTR_FULL(PAL0, 0, 0, 0, TILE_SYSTEMINDEX), col, row);
}
...
void RSE_clearTileRowAWithPrio(u16 row)
{
	...
	max_col = VDP_getPlanWidth(); 
	for(col = 0; col < max_col; col++)
		VDP_setTileMapXY(BG_A, TILE_ATTR_FULL(PAL0, TRUE, 0, 0, TILE_SYSTEMINDEX), col, row);
}

void RSE_resetScrolling(void)
{
	VDP_setVerticalScrollTile(BG_A, 0, (s16 *) tile_sc_table, 20, FALSE);	
	VDP_setVerticalScrollTile(BG_B, 0, (s16 *) tile_sc_table, 20, FALSE);
	VDP_setHorizontalScrollLine(BG_A, 0, (s16 *) tile_sc_table, 256, TRUE);
	VDP_setHorizontalScrollLine(BG_B, 0, (s16 *) tile_sc_table, 256, TRUE);
