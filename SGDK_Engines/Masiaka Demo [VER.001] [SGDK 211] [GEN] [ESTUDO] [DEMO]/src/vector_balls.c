...
NOINLINE void fastVectorBallFX(Sprite **prev_sprites)
{
	...
	inline auto void drawVectorBalls(u16 constant_angle, u16 accel_angle)
	{
		...
		_cosy = cosFix16(ry);
		_siny = sinFix16(ry);
		cs = F16_mul(_cosx, _siny);
		ss = F16_mul(_siny, _sinx);
		cc = F16_mul(_cosx, _cosy);
		sc = F16_mul(_sinx, _cosy);
		...
		for(loop = 0, shadow_idx = ball_count; loop < ball_count; loop++, shadow_idx++)
		{
		    ...
		    t_vtx[j].x = ((F16_mul(_vtx.x) + (_sinx)), F16_mul(_vtx.y, _cosx));
		    t_vtx[j].y = ((F16_mul(_vtx.x) - (cs)), ((F16_mul(_vtx.y) + (ss)), F16_mul(_vtx.z, _cosy)));
		    t_vtx[j].z = ((F16_mul(_vtx.x) - (cc)), F16_mul(_vtx.y, sc) - F16_mul(_vtx.z, _siny));
	...
	*/
	...
	VDP_setVerticalScroll(BG_A, -64);
	VDP_setVerticalScroll(BG_B, 0);
	VDP_setVerticalScrollTile(BG_A, 0, (s16*)palette_black, 20, FALSE);
	VDP_setVerticalScrollTile(BG_B, 0, (s16*)palette_black, 20, FALSE);
	...
	VDP_drawImageEx(BG_B, &checkboard_0, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, vramIndex), 0, (224 - 96) >> 3, FALSE, TRUE);
	vramIndex += checkboard_0.tileset->numTile;

	VDP_drawImageEx(BG_B, &checkboard_1, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, vramIndex), 80 >> 3, (224 - 96) >> 3, FALSE, TRUE);
	vramIndex += checkboard_1.tileset->numTile;

	VDP_drawImageEx(BG_B, &checkboard_2, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, vramIndex), 160 >> 3, (224 - 96) >> 3, FALSE, TRUE);
	vramIndex += checkboard_2.tileset->numTile;

	VDP_drawImageEx(BG_B, &checkboard_3, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, vramIndex), 240 >> 3, (224 - 96) >> 3, FALSE, TRUE);
	vramIndex += checkboard_3.tileset->numTile;

	VDP_drawImageEx(BG_B, &sky, TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, vramIndex), 0, ((224 - 96 - 72) >> 3), FALSE, TRUE);
	...
	screen corruption */
	{
		...
		for(row = 0; row < (224 - 96 - 72) >> 3; row++)
			for(col = 0; col < max_col; col++)
				VDP_setTileMapXY(BG_B, TILE_ATTR_FULL(PAL1, TRUE, FALSE, FALSE, TILE_SYSTEMINDEX), col, row);
	}
	...
	while(vball_phase < VBALL_PHASE_QUIT)
	{
		...
		switch(vball_phase)
		{
			case VBALL_PHASE_INTRO_SCROLL:
				...
				VDP_setVerticalScroll(BG_B, (easing_table[vball_timer << 4] >> 4) - 64);
				vball_timer++;
			...
			case VBALL_PHASE_INTRO_LOGO_FADEOUT:
				...
				if (music_sync > 650 && music_sync < 750)
				{
					...
					VDP_setHorizontalScrollLine(BG_B, 0, ramped_sine_scroll, 224, TRUE);
				}
				else
				if (music_sync > 750)
				{
			...
			case VBALL_PHASE_RUN:
				...
				{
					PAL_fadeOut(32, 32 + 15, RSE_FRAMES(32), TRUE);
					vball_timer = 0;
					vball_phase++;
				}
				break;
