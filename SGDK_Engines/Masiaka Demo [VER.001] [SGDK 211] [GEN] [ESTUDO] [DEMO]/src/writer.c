...
void updateScrollText(void)
{
	...
	if (scroll_local_offset >= letter_width)
	{
		...
		{
			if (c != ' ')
				VDP_setTileMapXY(current_plan, TILE_ATTR_FULL(current_pal, TRUE, FALSE, FALSE, TILE_USER_INDEX + charToTileIndex(c)), current_char_x, current_char_y); //TILE_ATTR_FULL(current_pal, FALSE, FALSE, FALSE, current_char_y));
			else
