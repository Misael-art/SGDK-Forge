...
void startGame()
{
	...
	VDP_fillTileMapRect(BG_B, TILE_ATTR_FULL(PAL1, 0, FALSE, FALSE, 1), 0, 10, 40, 10);
	gameOn = TRUE;
}
...
void checkPaddleBallCollision()
{
	...
	if (ballPosX < playerPosX + PLAYER_WIDTH && ballPosX + ballWidth > playerPosX)
	{
		...
		if (ballPosY < PLAYER_POS_Y + PLAYER_HEIGHT && ballPosY + ballHeight >= PLAYER_POS_Y)
		{
			...
			XGM_startPlayPCM(65, 1, SOUND_PCM_CH2);
			//SND_PCM4_startPlay(&hit, 9728, SOUND_PCM_CH1, SOUND_PCM_CH3);
...
void showLogo()
{
	...
	PAL_setPalette(PAL3, smbLogo.palette->data, DMA);
	VDP_fillTileMapRectInc(BG_A, TILE_ATTR_FULL(PAL3, 0, FALSE, FALSE, 3), 4, 9, logoTileW, logoTileH);

	PAL_fadePalette(PAL3, palette_black, smbLogo.palette->data, 60, 1);

	PAL_waitFadeCompletion();
	waitTick(TICKPERSECOND * 4);
	PAL_fadePalette(PAL3, smbLogo.palette->data, palette_black, 60, 1);
	PAL_waitFadeCompletion();
	initGame();
}
