...
int main() {
    ...
    while (1) {
        ...
        if (game_on && !game_paused) {
            ...
            SPR_setPosition(player,player_x,F16_toInt(player_y));//Atualiza a posição horizontal e vertical do sprite do jogador
            ...
            if(jumping == TRUE && F16_toInt(player_y)+player_height >= (floor_height)){//verifica se a posição y do jogador e maior ou igual à altura do solo
            ...
            player_y = FIX16(floor_height-player_height);//posiciona o jogador no solo caso o sprite nao fique alinhado com o terreno
