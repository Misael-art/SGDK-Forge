...
void loadTetrisPiecesGameplayGraphics() {
    ...
    PAL_setPalette(PALETTE_TETRIS_PIECE, bgtile.palette->data, DMA);
}

void loadTetrisGameplayBackgroundWithFadeIn() {
    PAL_setColors(PAL0, (u16*)palette_black, 64, DMA);

    PAL_setPalette(PALETTE_TETRIS_BACKGOUND, 
        tetris_scene_background.palette->data, DMA);
    ...
    PAL_fadeInAll(tetris_scene_background.palette->data, 40, FALSE);
