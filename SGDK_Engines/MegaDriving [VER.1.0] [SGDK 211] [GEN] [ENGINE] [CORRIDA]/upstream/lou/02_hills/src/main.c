...
HINTERRUPT_CALLBACK HIntHandler()
{
    ...
    lineGraphics = VscrollA[lineDisplay];
    VDP_setVerticalScroll(BG_A, lineGraphics);
    lineDisplay++;
}
...
void CreateHills(fix32 dy1, fix32 dy2, u16 segmentLine)
{
    ...
    char str_buffer[16];
    fix32ToStr(current_drawing_pos, str_buffer, 3); // The '2' is the number of decimal places
    kprintf("INIT:: %s", str_buffer);
    ...
    {
        s16 cdp = F32_toInt(current_drawing_pos); // get current drawing position as an int
kprintf("current_drawing_position: %d", cdp);
        ...
        s16 ndp = F32_toInt(next_drawing_pos);
...
int main(bool hard)
{
    ...
    //jCreateHills(FIX32(0), FIX32(0.04), 145);
    CreateHills(FIX32(0), FIX32(0), 0);
