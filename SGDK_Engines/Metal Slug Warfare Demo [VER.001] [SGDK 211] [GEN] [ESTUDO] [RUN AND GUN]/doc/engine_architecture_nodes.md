# Engine Architecture Nodes - Metal Slug Warfare Demo [VER.001] [SGDK 211] [GEN] [ESTUDO] [RUN AND GUN]

Overview of the technical structure of the Metal Slug Warfare Demo [VER.001] [SGDK 211] [GEN] [ESTUDO] [RUN AND GUN] engine.

## 1. Modular Structure
The engine is composed of the following core modules:
- **`main.c`**: Entry point and primary game loop.
- **`Animations.c`**: Module file.
- **`GestionPAD.c`**: Module file.
- **`Init.c`**: Module file.
- **`Main.c`**: Module file.
- **`Routines.c`**: Module file.
- **`Variables.c`**: Module file.
