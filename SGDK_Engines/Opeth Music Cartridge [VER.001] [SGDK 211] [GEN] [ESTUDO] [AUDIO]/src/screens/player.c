...
{    
    ...
    VDP_drawImageEx(WINDOW, &art, TILE_ATTR_FULL(PAL2, TRUE, FALSE, FALSE, TILE_USER_INDEX + bg.tileset->numTile), 21, 0, FALSE, TRUE);
...
static void preparePlayerState(u16 i){
    ...
    // init some GFX var
    tileIndexProgressBar = TILE_USER_INDEX + bg.tileset->numTile + art_damnation.tileset->numTile;
    tileIndex = TILE_USER_INDEX + bg.tileset->numTile + art_damnation.tileset->numTile + progress_bar.tileset->numTile + starfield.tileset->numTile;
    ...
    trackListShadowTop[0] = SPR_addSpriteEx(&shadow_mask_16, 8, 100, TILE_ATTR_FULL(PAL3, TRUE, FALSE, FALSE, curTileIndexMesh), 0, SPR_FLAG_AUTO_VRAM_ALLOC);
    trackListShadowTop[1] = SPR_addSpriteEx(&shadow_mask_16, 8+128, 100, TILE_ATTR_FULL(PAL3, TRUE, FALSE, FALSE, curTileIndexMesh), 0, SPR_FLAG_AUTO_VRAM_ALLOC);
    trackListShadowTop[2] = SPR_addSpriteEx(&shadow_mask_7, 8+256, 100, TILE_ATTR_FULL(PAL3, TRUE, FALSE, FALSE, curTileIndexMesh), 0, SPR_FLAG_AUTO_VRAM_ALLOC);
    trackListShadowBottom[0] = SPR_addSpriteEx(&shadow_mask_16, 8, 132, TILE_ATTR_FULL(PAL3, TRUE, TRUE, FALSE, curTileIndexMesh), 0, SPR_FLAG_AUTO_VRAM_ALLOC);
    trackListShadowBottom[1] = SPR_addSpriteEx(&shadow_mask_16, 8+128, 132, TILE_ATTR_FULL(PAL3, TRUE, TRUE, FALSE, curTileIndexMesh), 0, SPR_FLAG_AUTO_VRAM_ALLOC);
    trackListShadowBottom[2] = SPR_addSpriteEx(&shadow_mask_7, 8+256, 132, TILE_ATTR_FULL(PAL3, TRUE, TRUE, FALSE, curTileIndexMesh), 0, SPR_FLAG_AUTO_VRAM_ALLOC);
    for(i = 0; i < 3; i++)
    {
...
static void drawStaticGUI()
{
    ...
    VDP_drawImageEx(WINDOW, &art_damnation, TILE_ATTR_FULL(PAL2, TRUE, FALSE, FALSE, TILE_USER_INDEX + bg.tileset->numTile), 21, 0, FALSE, TRUE);
    VDP_drawImageEx(WINDOW, &progress_bar, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, tileIndexProgressBar), 9, 8, FALSE, TRUE);
    
    // starfield
    //VDP_drawImageEx(BG_B, &starfield, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, TILE_USER_INDEX + bg.tileset->numTile + art_damnation.tileset->numTile + progress_bar.tileset->numTile), 0, 0, FALSE, TRUE);
...
void hint()
{
 ...
 void playerScreen(){
    ...
    //PAL_setPalette(PAL1, aux_pal.palette->data, DMA);
    //PAL_setPalette(PAL1,  , DMA);
    u16 pal_transf[16];
