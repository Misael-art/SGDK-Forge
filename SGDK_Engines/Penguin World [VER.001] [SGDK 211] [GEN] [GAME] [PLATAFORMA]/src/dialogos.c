...
void dialogo(u16 x,u16 y,u8 ancho, u8 alto, u8 numdiag){
	...
	//--------Marco Esquinas---------------------------------------------------------
	Sprite* dig_marcoE[4];
	dig_marcoE[0]=SPR_addSpriteEx(&dig_marco1,x,y,TILE_ATTR_FULL(2,TRUE,FALSE,FALSE,diag_ind), SPR_FLAG_AUTO_VRAM_ALLOC);
	dig_marcoE[1]=SPR_addSpriteEx(&dig_marco1,x+8+(32*ancho),y,TILE_ATTR_FULL(2,TRUE,FALSE,TRUE,diag_ind), SPR_FLAG_AUTO_VRAM_ALLOC);
	dig_marcoE[2]=SPR_addSpriteEx(&dig_marco1,x,y+8+(32*alto),TILE_ATTR_FULL(2,TRUE,TRUE,FALSE,diag_ind), SPR_FLAG_AUTO_VRAM_ALLOC);
	dig_marcoE[3]=SPR_addSpriteEx(&dig_marco1,x+8+(32*ancho),y+8+(32*alto),TILE_ATTR_FULL(2,TRUE,TRUE,TRUE,diag_ind), SPR_FLAG_AUTO_VRAM_ALLOC);
	...
	for(i=0;i<ancho;i++){
		x2=x+8+(32*i);
		dig_marcoH [i]=SPR_addSpriteEx(&dig_marco2,x2,y ,TILE_ATTR_FULL(2,TRUE,FALSE,FALSE,diag_ind+1), SPR_FLAG_AUTO_VRAM_ALLOC);
		dig_marcoHd[i]=SPR_addSpriteEx(&dig_marco2,x2,y2,TILE_ATTR_FULL(2,TRUE,TRUE ,FALSE,diag_ind+1), SPR_FLAG_AUTO_VRAM_ALLOC);
	...
	for(i=0;i<alto;i++){
		y2=y+8+(32*i);
		dig_marcoV [i]=SPR_addSpriteEx(&dig_marco3,x ,y2,TILE_ATTR_FULL(2,TRUE,FALSE,FALSE,diag_ind+1+4), SPR_FLAG_AUTO_VRAM_ALLOC);
		dig_marcoVd[i]=SPR_addSpriteEx(&dig_marco3,x2,y2,TILE_ATTR_FULL(2,TRUE,FALSE,TRUE ,diag_ind+1+4), SPR_FLAG_AUTO_VRAM_ALLOC);
	...
	for(u8 v=0;v<alto;v++){
		for(u8 h=0;h<ancho;h++){
			dig_lienzo[spritelienzo3232]=SPR_addSpriteEx(&dig_marco4,x+8+(32*h),y+8+(32*v),TILE_ATTR_FULL(2,TRUE,FALSE,FALSE,Start_ind+(16*spritelienzo3232)), SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);		
