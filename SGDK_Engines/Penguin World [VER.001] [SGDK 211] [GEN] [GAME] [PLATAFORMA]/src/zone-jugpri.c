...
void ZoneMap(){
	...
	Sprite* dig_temp[3];
	dig_temp[0]=SPR_addSpriteEx(&dig_marco1, 0, 0, diag_ind, SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
	dig_temp[1]=SPR_addSpriteEx(&dig_marco2, 0, 0, diag_ind+1, SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
	dig_temp[2]=SPR_addSpriteEx(&dig_marco3, 0, 0, diag_ind+1+4, SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
	...
	penguinsp=SPR_addSpriteEx(&penguin, 160-12, ScreenMY-18, TILE_ATTR(1,jugpri,FALSE,FALSE), SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
	...
	if(CursorON){ 
		cursorsp=SPR_addSpriteEx(&cursor, 160, ScreenMY, TILE_ATTR(0,TRUE,FALSE,FALSE), SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | SPR_FLAG_INSERT_HEAD);
...
static void SPR_PRIORITY(){
	...
	if(zona1dat[ZONA_NUM].top_blxpri>0){
		...
		if(jugpri!=jugpricpy){ jugpricpy=jugpri; 
			SPR_setPriority(penguinsp,jugpri);
		}
	}
