/*
 * =============================================================================
 * SHADOW DANCER - Engine de ação/plataforma para Mega Drive (SGDK 2.11)
 * =============================================================================
 *
 * PROPÓSITO PEDAGÓGICO (MegaDrive_DEV):
 *   Este código serve como material de estudo: demonstra um jogo completo
 *   (personagem, câmera, colisão, FSM, tilesets dinâmicos) compatível com
 *   SGDK 2.11. O conteúdo está comentado para facilitar o aprendizado de
 *   desenvolvimento para Mega Drive / Genesis.
 *
 * CRÉDITOS:
 *   Projeto: GameDevBoss
 *   Música: Edmo Caldas
 *   Arte/Gfx: Ray Castello, Gabriel Pyron
 *   Sprites Ninja originais: Vubidugil
 *
 * ESTRUTURA DO ARQUIVO (ordem de leitura sugerida):
 *   1. Constantes e globais  - limites do jogo e resolução VDP
 *   2. Structs              - Player, inimigos, projéteis, efeitos
 *   3. Declarações de funções- índice das rotinas do engine
 *   4. main()                - inicialização VDP e loop principal (rooms)
 *   5. PLAYER_STATE / FSM    - máquina de estados do jogador
 *   6. ANIMATION / PHYSICS   - animação e movimento
 *   7. COLLISION_HANDLING    - resposta a colisões com o mapa
 *   8. CAMERA                - scroll e posição na tela
 *   9. CREATE_STAGE          - carregamento de fases e caixas de colisão
 *  10. UPDATE_* / DRAW_*     - lógica e desenho de objetos (shuriken, bombas, etc.)
 *
 * SGDK: compatível com SGDK 2.11 (genesis.h, VDP, SPR, MAP, PAL, XGM).
 * =============================================================================
 */
 ...
 * CONSTANTES DO JOGO
 * -----------------------------------------------------------------------------
 * Câmera: distância em pixels do jogador à borda da tela que dispara o scroll.
 * Valores em 320x224 (resolução padrão do Mega Drive) para movimento suave.
 * Resolução: HORIZONTAL_RESOLUTION x VERTICAL_RESOLUTION = tamanho da tela VDP.
 * SFX: canais PCM para efeitos (65, 66, 67 reservados para jogador/CPU).
 * Limites: máximos de objetos ativos (shurikens, bombas, inimigos, etc.).
 * -----------------------------------------------------------------------------
 */
 ...
 * VARIÁVEIS GLOBAIS
 * -----------------------------------------------------------------------------
 * gRoom: "sala" do jogo (menu=3, gameplay=10, level select=9, descompressão=0, etc.).
 * gInd_tileset: índice acumulado ao carregar tilesets (VDP_loadTileSet) para
 *   não sobrescrever tiles; cada mapa soma numTile ao índice.
 * level_map (BG_A) / level_mapb (BG_B): planos de fundo; scroll via MAP_scrollTo.
 * collisionMatrix: caixas de colisão do cenário [id][x1,y1,x2,y2,plano].
 * collisionMatrixB: áreas de troca de plano (subir/descer) [x1,y1,x2,y2,tipo].
 * gPing2/gPing10/gPing20/gPing60: contadores cíclicos (0..N) usados para
 *   timers baratos e animações em intervalos (ex.: 10% do tempo, 1/60 s).
 * -----------------------------------------------------------------------------
 */
u8   i;
u32  gFrames = 0;
...
u8 gSubMapID = 1;
u16 gItemID;
s16 collisionMatrix[TOTAL_MAP_BOXES][5];
s16 collisionMatrixB[TOTAL_MAP_BOXES/4][5];
s16 totalMapCollisionBoxes;
s16 totalMapCollisionBoxesB;
...
/*
 * PlayerDEF: dados do jogador (e slots P[0], P[1], P[2] para CPU/jogador).
 * state: ID do estado na FSM (100=parado, 101=shuriken, 200=agachado, 300=jump, etc.).
 * impulse/velocity: física simples; jumpTimer controla duração do pulo.
 * groundSensor: true quando em contato com chão (usado em PHYSICS e COLLISION_HANDLING).
 * axisX/axisY: pivot do sprite para posicionar na tela (câmera subtrai posição do mapa).
 * dataAnim[]/animRow: definem duração de cada frame e linha no atlas; FSM altera state.
 * DM (Directional Movement): direção do movimento no momento da colisão (teclado numérico 1-9).
 */
struct PlayerDEF {
	Sprite* sprite;
	u16 state;
...
/* Objetos de animação decorativa (ex.: água, fogueira) posicionados no mapa. */
struct AnimObjDEF {
	Sprite* sprite;
	s16 x;
	s16 y;
...
/* Efeitos visuais (fumaça, impacto); life decrementa a cada frame até desativar. */
struct fxobjsDEF {
	Sprite* sprite;
	u16 x;
	u16 y;
...
/* Shuriken que atingiu algo e vira efeito "quebrado" antes de sumir. */
...
/* Bombas ativas no cenário; explodem após life ou contato. */
struct bombsDEF {
	Sprite* sprite;
	u16 x;
	u16 y;
...
/* Shurikens em voo; myLayer para colisão com planos (BG_A/BG_B). */
...
/* Inimigos; state/type definem IA e sprite; layer = plano do cenário. */
struct enemysDEF {
	Sprite* sprite;
	u16 x;
	u16 y;
...
/* Itens/ seeds no mapa (coletáveis). */
struct seedDEF {
	u16 x;
	u16 y;
	u8 type;
...
/* -----------------------------------------------------------------------------
 * DECLARAÇÕES DE FUNÇÕES (índice do engine)
 * -----------------------------------------------------------------------------
 * Jogador: PLAYER_STATE, INPUT_SYSTEM, FSM, ANIMATION, PHYSICS, JUMP_INIT, COLLISION_HANDLING.
 * Sistema: CAMERA, CLEAR_VDP, CREATE_STAGE, DYNAMIC_LOADING_TILESET, CHECK_COLLISION.
 * Objetos: FXOBJECT, BOMB, OBJSHURIKENS, OBJSHURIKENSDEAD, ENEMYS, OBJ_SEED + UPDATE_* / DRAW_*.
 * Áudio: PLAY_SND. HUD: UPDATE_SCORE.
 * -----------------------------------------------------------------------------
 */
...
/*
 * main()
 * - Inicializa VDP (resolução 320x224, planos 64x32), sprites (SPR_initEx) e cor de fundo.
 * - Loop infinito: a cada frame atualiza gFrames, gPing*, lê INPUT_SYSTEM e, se gRoom==10
 *   (gameplay), executa FSM -> ANIMATION -> PHYSICS -> CAMERA e depois UPDATE/DRAW de
 *   efeitos, bombas, shurikens, inimigos, tileset dinâmico e HUD.
 * - Outros gRoom (0, 3, 9, etc.) tratam menu, level select, descompressão, etc.
 */
int main(bool hardReset)
{
	/* Inicialização VDP (SGDK 2.11): desabilitar interrupções durante setup. */
	SYS_disableInts();
	VDP_init();
	...
	while (TRUE)
	{
		gFrames++;
		if (gPauseSystem == 0) {
			if (gPing2  == 1)  { gPing2  = -1; } gPing2++;
			if (gPing10 == 9)  { gPing10 = -1; } gPing10++;
			if (gPing20 == 19) { gPing20 = -1; } gPing20++;
			if (gPing60 == 59) { gPing60 = -1; } gPing60++;
		}

		INPUT_SYSTEM();

		/* Room 10 = gameplay: FSM (estados do jogador), animação, física, câmera e objetos. */
		if (gRoom == 10)
		{
			FSM();
			ANIMATION();
		...
		if(gRoom==4)
		{
			if(gFrames==1)
			{
				...
				AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_txt_tgnr, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				AnimObj[0].x = -152;
				AnimObj[0].y = 16;
		...
		if(gRoom==5)
		{
			if(gFrames==1)
			{
				...
				P[0].sprite = SPR_addSpriteExSafe(&spr_ninja_logo, 18, 72, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				if(P[0].sprite){ SPR_setVRAMTileIndex(P[0].sprite, 1441); } //define uma posicao especifica para o GFX na VRAM
				
				P[1].sprite = SPR_addSpriteExSafe(&spr_press_start, 88, 20, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		...
		if(gRoom==8)
		{
			if(gFrames==1)
			{
				...
				P[1].sprite = SPR_addSpriteExSafe(&spr_charsel_shinobi2,  64, 56, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				P[2].sprite = SPR_addSpriteExSafe(&spr_charsel_shinobi1, 184, 56, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
			...
			if(gFrames==4)
			{
				P[0].sprite = SPR_addSpriteExSafe(&spr_char_sel_cursor_new_fx,  91, 197, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				SPR_setVRAMTileIndex(P[0].sprite, 1441); //define uma posicao especifica para o GFX na VRAM
				
				AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_char_sel_titleA, 320, 224, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				AnimObj[1].sprite = SPR_addSpriteExSafe(&spr_char_sel_titleB, 320, 224, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
			...
			if(gFrames==6)
			{
				...
				XGM_isPlaying();
				AnimObj[2].sprite = SPR_addSpriteExSafe(&spr_char_sel_cursor_new_fx, 91-43, 197, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		...
		if(gRoom==9)
		{
			if(gFrames==1)
			{
				...
				if(P[0].sprite){ SPR_releaseSprite(P[0].sprite); P[0].sprite = NULL; } //clear memory
				P[0].sprite = SPR_addSpriteExSafe(&spr_stage_sel_cursor, 136, 40, TILE_ATTR(PAL0, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
			}
		...
		if(gRoom==10)
		{
			...
			if(gFrames==2) //HUD
			{
				hudElement[0] = SPR_addSpriteExSafe(&spr_HUD_face,   8,  0, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[1] = SPR_addSpriteExSafe(&spr_HUD_lives_numbers,  56, 16, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				
				hudElement[4] = SPR_addSpriteExSafe(&spr_HUD_bomb, 232,  0, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[5] = SPR_addSpriteExSafe(&spr_HUD_numbers, 296,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[6] = SPR_addSpriteExSafe(&spr_HUD_magic, 0, 195, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				
				//2:59 clock
				hudElement[ 7] = SPR_addSpriteExSafe(&spr_HUD_numbers, 168,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				if(hudElement[ 7]){ SPR_setAnimAndFrame(hudElement[ 7], 0, 2); }
				hudElement[ 8] = SPR_addSpriteExSafe(&spr_HUD_numbers, 176,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				if(hudElement[ 8]){ SPR_setAnimAndFrame(hudElement[ 8], 0, 10); } //: << 2:59
				hudElement[ 9] = SPR_addSpriteExSafe(&spr_HUD_numbers, 184,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				if(hudElement[ 9]){ SPR_setAnimAndFrame(hudElement[ 9], 0, 5); }
				hudElement[10] = SPR_addSpriteExSafe(&spr_HUD_numbers, 192,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				if(hudElement[10]){ SPR_setAnimAndFrame(hudElement[10], 0, 9); }
				
				//points
				hudElement[11] = SPR_addSpriteExSafe(&spr_HUD_points,  40,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[12] = SPR_addSpriteExSafe(&spr_HUD_points,  48,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[13] = SPR_addSpriteExSafe(&spr_HUD_points,  56,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[14] = SPR_addSpriteExSafe(&spr_HUD_points,  64,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[15] = SPR_addSpriteExSafe(&spr_HUD_points,  72,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				hudElement[16] = SPR_addSpriteExSafe(&spr_HUD_points,  80,  8, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		...
		if(gRoom==11)
		{
			if(gFrames==1)
			{
				...
				if(gMapID>=1 && gMapID<=3)
				{
					...
					P[0].sprite = SPR_addSpriteExSafe(&spr_log_mission1     ,  48,   8, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[1].sprite = SPR_addSpriteExSafe(&spr_log_mission1_txtA,   0, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[2].sprite = SPR_addSpriteExSafe(&spr_log_mission1_txtB, 160, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				}
				if(gMapID>=4 && gMapID<=7)
				{
					...
					P[0].sprite = SPR_addSpriteExSafe(&spr_log_mission2     ,  48,   8, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[1].sprite = SPR_addSpriteExSafe(&spr_log_mission2_txtA,   0, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[2].sprite = SPR_addSpriteExSafe(&spr_log_mission2_txtB, 160, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				}
				if(gMapID>=8 && gMapID<=11)
				{
					...
					P[0].sprite = SPR_addSpriteExSafe(&spr_log_mission3     ,  48,   8, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[1].sprite = SPR_addSpriteExSafe(&spr_log_mission3_txtA,   0, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[2].sprite = SPR_addSpriteExSafe(&spr_log_mission3_txtB, 160, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				}
				if(gMapID>=12 && gMapID<=15)
				{
					...
					P[0].sprite = SPR_addSpriteExSafe(&spr_log_mission4     ,  48,   8, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[1].sprite = SPR_addSpriteExSafe(&spr_log_mission4_txtA,   0, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
					P[2].sprite = SPR_addSpriteExSafe(&spr_log_mission4_txtB, 160, 176, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				}
				...
				if(gMapID==1){AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_log_indicator11, 320, 224, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );}
				if(gMapID==2){AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_log_indicator12, 320, 224, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );}
				if(gMapID==3){AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_log_indicator13, 320, 224, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );}
		...
		//--- FINALIZATIONS ---//
		//VDP_showFPS(1, 1, 1); //Shows FPS rate
 ...
 * Define o estado do jogador na FSM e a animação correspondente no atlas.
 * State segue convenção de teclado numérico (ex.: 100=parado, 101=shuriken, 200=agachado,
 * 300=pulo, 400/401=andar, 606=pouso). dataAnim[] e animRow definem frames e linha do sprite.
 */
void PLAYER_STATE(u8 Player, u16 State)
{
...
/*
 * FSM() - Máquina de estados do jogador (P[1]).
 * A cada frame, conforme state atual e entradas (key_JOY_*), dispara transições
 * (PLAYER_STATE) e ações (OBJSHURIKENS no frame certo da animação). Estados
 * principais: 100 parado, 101/201 shuriken, 200 agachado, 300/310 pulo, 400/401 andar.
 */
void FSM()
{
	/* Disparo de shuriken no frame 3 da animação de ataque. */
...
/*
 * COLLISION_HANDLING(player, RectX1, RectY1, RectX2, RectY2)
 * Ajusta posição do jogador após colisão com uma caixa (Rect) do mapa.
 * DM (Directional Movement) indica de que direção o jogador entrou (8=cima, 2=baixo,
 * 4=esq, 6=dir, 1/3/7/9 diagonais). Corrige x ou y para não atravessar; em contato
 * com chão (DM 1/2/3) pode transitar para estado de pouso (606) e resetar física.
 */
void COLLISION_HANDLING(u8 player, s32 RectX1, s32 RectY1, s32 RectX2, s32 RectY2)
{
	/* Centro da caixa e do jogador para decidir correção vertical vs horizontal em diagonais. */
...
/*
 * CAMERA()
 * Câmera que segue o jogador: calcula nova posição (new_camera_x/y) quando o jogador
 * ultrapassa os limites HOW_FAR_*; aplica MAP_scrollTo em level_map e level_mapb
 * (BGB com scroll /2 para parallax no mapa 13); atualiza posição do sprite do
 * jogador e dos AnimObj em coordenadas de tela (mapa - câmera).
 */
void CAMERA()
{
	int player_x_map_integer = P[1].x;
	int player_y_map_integer = P[1].y + 5;
	...
	/* Nova posição da câmera quando o jogador sai da zona morta. */
...
/*
 * CLEAR_VDP()
 * Limpa VDP e estado do jogo: reseta sprites, planos A/B, scroll, paleta preta,
 * libera level_map/level_mapb e sprites do jogador/efeitos/inimigos. Usado em
 * transições de tela e fake reset (volta ao level select).
 */
void CLEAR_VDP()
{
	SYS_disableInts();
...
/*
 * CHECK_COLLISION(R1x1, R1y1, R1x2, R1y2, R2x1, R2y1, R2x2, R2y2)
 * Retorna true se os retângulos R1 (x1,y1,x2,y2) e R2 se sobrepõem.
 * Detecção AABB: R1 à esquerda de R2 e R2 à esquerda de R1 em X e Y.
 */
bool CHECK_COLLISION(s32 R1x1, s32 R1y1, s32 R1x2, s32 R1y2, s32 R2x1, s32 R2y1, s32 R2x2, s32 R2y2)
{
...
/*
 * CREATE_STAGE(mapID)
 * Carrega uma fase: paleta, tilesets (VDP_loadTileSet), mapas BG_A/BG_B (MAP_create),
 * caixas de colisão (collisionMatrix) e áreas de troca de plano (collisionMatrixB).
 * mapID 0 apenas zera as matrizes; mapID >= 1 define gBG_Width/Height, posição do
 * jogador, bombas, e preenche collisionMatrix/collisionMatrixB com retângulos (x1,y1,x2,y2,plano).
 */
void CREATE_STAGE(s16 mapID)
{
	s16 a = 0, b = 0, c = 0, d = 0, e = 0;

	gInd_tileset = 0;
	PAL_setColors(0, palette_black, 64, DMA);
	VDP_setBackgroundColor(0);

	if (mapID == 0) /* Apenas reset das matrizes de colisão. */
	{
	...
	if(mapID==1) //LVL1-1
	{
		...
		//detector metais
		AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_lvl11_anim_obj1, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		AnimObj[0].x = 320;
		AnimObj[0].y = 144;
		if(AnimObj[0].sprite){ SPR_setDepth(AnimObj[0].sprite, 1); }
		
		//monitor
		AnimObj[1].sprite = SPR_addSpriteExSafe(&spr_lvl11_anim_obj2, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		AnimObj[1].x = 336;
		AnimObj[1].y = 144;
		if(AnimObj[1].sprite){ SPR_setDepth(AnimObj[1].sprite, 255); }
		
		//telao
		//AnimObj[2].sprite = SPR_addSpriteExSafe(&spr_lvl11_anim_obj3, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		//AnimObj[2].x =  48;
		//AnimObj[2].y =  32;
		//if(AnimObj[1].sprite){ SPR_setDepth(AnimObj[2].sprite, 255); }
		
		//in
		AnimObj[3].sprite = SPR_addSpriteExSafe(&spr_lvl11_anim_obj4, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		AnimObj[3].x = 1968;
		AnimObj[3].y =  152;
	...
	if(mapID==2) //LVL1-2
	{
		...
		//escada aviao
		AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_element_lvl12A, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		AnimObj[0].x = 1648;
		AnimObj[0].y = 176;
		if(AnimObj[0].sprite){ SPR_setDepth(AnimObj[0].sprite, 1); }
	...
	if(mapID==4) //LVL2-1
	{
		...
		// ANIMATED OBJECTS AND OTHERS //
		//caminhaoA
		AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_element_lvl21A, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		AnimObj[0].x = 1480;
		AnimObj[0].y = 144;
		if(AnimObj[0].sprite){ SPR_setDepth(AnimObj[0].sprite, 255); }
		//caminhaoB
		AnimObj[1].sprite = SPR_addSpriteExSafe(&spr_element_lvl21B, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		AnimObj[1].x = 1632+16;
		AnimObj[1].y = 144+16;
		if(AnimObj[1].sprite){ SPR_setDepth(AnimObj[1].sprite, 255); }
		//caminhaoC
		//AnimObj[2].sprite = SPR_addSpriteExSafe(&spr_element_lvl21C, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		//AnimObj[2].x = 1784;
		//AnimObj[2].y = 144;
	...
	if(mapID==14) //LVL4-3
	{
		...
		// ANIMATED OBJECTS AND OTHERS //
		//plataformA
		//AnimObj[0].sprite = SPR_addSpriteExSafe(&spr_element_lvl43A, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		//AnimObj[0].x = 744;
		//AnimObj[0].y = 160;
		//if(AnimObj[0].sprite){ SPR_setDepth(AnimObj[0].sprite, 255); }
		//plataformB
		//AnimObj[1].sprite = SPR_addSpriteExSafe(&spr_element_lvl43A, -99, -99, TILE_ATTR(PAL1, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
		//AnimObj[1].x = 896;
		//AnimObj[1].y = 160;
...
void FXOBJECT(s32 x, s32 y, u8 dir)
{
    for (int i = 0; i < MAX_SMOKE_FX; i++) {
        if (!fxobjs[i].active) {
			...
			fxobjs[i].sprite = SPR_addSpriteExSafe(&spr_smoke_fx, 320, 224, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
...
void BOMB(s32 x, s32 y, u8 dir)
{
    for (int i = 0; i < MAX_BOMBS; i++) {
        if (!bombs[i].active) {
			...
			if(bombs[i].dir!=99)
			{
				bombs[i].sprite = SPR_addSpriteExSafe(&spr_bomb, 320, 224, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
				SPR_setVRAMTileIndex(bombs[i].sprite, 1441); //define uma posicao especifica para o GFX na VRAM
			}else{
				bombs[i].sprite = SPR_addSpriteExSafe(&spr_bomb_dead, 320, 224, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );  //bomb dead
			}
...
void OBJSHURIKENS(s32 x, s32 y, u8 dir)
{
    for (int i = 0; i < MAX_SHURIKENS; i++) {
        if (!obj_shuriken[i].active) {
			...
			//load sprite outside of the view (320,224)
			obj_shuriken[i].sprite = SPR_addSpriteExSafe(&spr_shuriken, 320, 224, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
...
void OBJSHURIKENSDEAD(s32 x, s32 y, u8 dir)
{
    for (int i = 0; i < MAX_SHURIKENS; i++) {
        if (!obj_shurikendead[i].active) {
			...
			//load sprite outside of the view (320,224)
			obj_shurikendead[i].sprite = SPR_addSpriteExSafe(&spr_shuriken_dead2, 320, 224, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 );
...
void ENEMYS(s32 x, s32 y, u8 type, s8 dir, u8 layer)
{
    for (int i = 0; i < MAX_ENEMYS; i++) {
        if (!enemys[i].active) {
			...
			//load sprite outside of the view (320,224)
			if(type==1){ enemys[i].sprite = SPR_addSpriteExSafe(&spr_enemy01, 320, 224, TILE_ATTR(PAL3, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 ); }
			if(type==2){ enemys[i].sprite = SPR_addSpriteExSafe(&spr_enemy02, 320, 224, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 ); }
			if(type==3){ enemys[i].sprite = SPR_addSpriteExSafe(&spr_enemy03, 320, 224, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 ); }
			if(type==4){ enemys[i].sprite = SPR_addSpriteExSafe(&spr_enemy04, 320, 224, TILE_ATTR(PAL2, TRUE, FALSE, FALSE), SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD | 0x800 ); }
			if(enemys[i].sprite)
			{ 
