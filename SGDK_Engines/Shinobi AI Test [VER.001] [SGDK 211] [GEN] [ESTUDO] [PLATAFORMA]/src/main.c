...
int main()
{
    ...
    PAL_setPalette(PAL2, red_shinobi_sprite.palette->data, DMA);
    PAL_setPalette(PAL3, bomber_sprite.palette->data, DMA);

    SPR_init();
    player = SPR_addSprite(&red_shinobi_sprite, playerX, F16_toInt(playerY), TILE_ATTR(PAL2, TRUE, FALSE, FALSE));
...
static void handleInput()
{
    ...
    if (jumping == TRUE && F16_toInt(playerY) + player_height >= floor_height) 
    {
        jumping = FALSE;
        player_vel_y = FIX16(0);
        playerY = FIX16(floor_height - player_height);
        SPR_setAnim(player, ANIM_STAND);
    }
    ...
    setSpritePosition(player, playerX - camPosX, F16_toInt(playerY) - camPosY);
...
//There are 5 different coloured ninjas in the game, and this will allow cycling through them by pushing the X button (only green and red ninjas included for now)
{
        ...
        if (shinobiC == 0) {
            ...
            PAL_setPalette(PAL2, green_shinobi_sprite.palette->data, DMA);
            player = SPR_addSprite(&green_shinobi_sprite, playerX, F16_toInt(playerY), TILE_ATTR(PAL2, 0, FALSE, FALSE));
            if (faceLeft == TRUE) SPR_setHFlip(player, TRUE);
        }
        else if (shinobiC == 1) {
            SPR_releaseSprite(player);
            PAL_setPalette(PAL2, red_shinobi_sprite.palette->data, DMA);
            player = SPR_addSprite(&red_shinobi_sprite, playerX, F16_toInt(playerY), TILE_ATTR(PAL2, 0, FALSE, FALSE));
            if (faceLeft == TRUE) SPR_setHFlip(player, TRUE);
        }
        swapButton = FALSE;
