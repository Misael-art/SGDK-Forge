...
int main()
{
    ...
    PAL_setPalette(PAL0, highway.palette->data, DMA);
    PAL_setPalette(PAL2, cars.palette->data, DMA);
    ind = TILE_USER_INDEX;
    ...
    PAL_setPalette(PAL1, shinobi_sprite_green.palette->data, DMA);
...
static void changeShinobi()
{
        ...
        if (shinobiC == 0) {
            ...
            SPR_releaseSprite(player);
            PAL_setPalette(PAL1, shinobi_sprite_red.palette->data, DMA);
