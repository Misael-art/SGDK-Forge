...
void Board_RedrawGameFiled(Board *board)
{
    ...
    for (s16 y = 0; y < BOARD_HEIGHT - 2; y++)
    {
        ...
        {
            tiles[y][x] = TILE_ATTR_FULL(PAL1,0,FALSE,FALSE,TILE_USER_INDEX + board->data[y][x + 2]);
        }

    }
    SYS_disableInts();
    VDP_setTileMapDataRect(BG_A, tiles[0], board->xTileOnScreen + 2, board->yTileOnScreen, BOARD_WIDTH - 4, BOARD_HEIGHT - 2, BOARD_WIDTH - 4, DMA);
    SYS_enableInts();
}
