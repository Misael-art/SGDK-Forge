...
void GameTitle()
{
    ...
    VDP_loadTileSet(titleImage.tileset,TILE_USER_INDEX,DMA);
	PAL_setPalette(PAL1, titleImage.palette->data, DMA);
	VDP_setMap(BG_A, titleImage.map, TILE_ATTR_FULL(PAL1, 0, 0, 0, TILE_USER_INDEX), 0, 0);
	SYS_enableInts();

    JOY_init();


    XGM_startPlay(title_xgm);
...
void GameOption()
{
    ...
    VDP_clearPlane(BG_A, TRUE);
    VDP_clearPlane(BG_B, TRUE);
    PAL_setPalette(PAL2, optionSprite.palette->data, DMA);
...
void GamePlay()
{
    ...
    VDP_loadTileSet(boardImage.tileset,TILE_USER_INDEX,DMA);
	PAL_setPalette(PAL1, boardImage.palette->data, DMA);

    VDP_loadTileSet(bg2Image.tileset, TILE_USER_INDEX + 10, DMA);
    PAL_setPalette(PAL3, bg2Image.palette->data, DMA);


    VDP_setMap(BG_B, bg2Image.map, TILE_ATTR_FULL(PAL3, 0, 0, 0, TILE_USER_INDEX + 10), 0, 0);
...
void joyEventOnGameTitle(u16 joy, u16 changed, u16 state)
{
    if (changed & state & BUTTON_START)
    {
        ...
        XGM_startPlayPCM(SOUND_START, 10, SOUND_PCM_CH2);
        //XGM_stopPlay();
    }
}
...
void joyEventOnGamePlay(u16 joy, u16 changed, u16 state) //state = pressed , changed = released
{
    ...
    if (changed & state & BUTTON_START)
    {
        ...
        if (paused)
        {
            ...
            XGM_pausePlay();
            XGM_startPlayPCM(SOUND_PAUSE, 10, SOUND_PCM_CH2);
        }
        else
        {
            ShowPauseSprite(paused);
    ...
    switch(block1.state)
    {
        ...
        case LANDING:
            ...
            else if (block1.landDelay == 0)
            {
                ...
                if (Board_FilledLineCount(&block1, &board1) != 0)
                {
                    ...
                    //播放消行动画
                     XGM_startPlayPCM(SOUND_LINE_CLEAR, 10, SOUND_PCM_CH2);
        ...
        case REBORN:
            ...
            else
            {
                ...
                XGM_stopPlay();
                XGM_startPlayPCM(SOUND_GAME_OVER, 10, SOUND_PCM_CH2);
...
void BlockLock()
{
    ...
    XGM_startPlayPCM(SOUND_FALL, 10, SOUND_PCM_CH2);
    ...
    PAL_setPalette(PAL2, palette_white, DMA);
    block1.landDelay = 10;
...
void SoundInit()
{
    XGM_setPCM(SOUND_FALL, fall_wav, sizeof(fall_wav));
    XGM_setPCM(SOUND_HOLD, hold_wav, sizeof(hold_wav));
    XGM_setPCM(SOUND_PAUSE, pause_wav, sizeof(pause_wav));
    XGM_setPCM(SOUND_LINE_CLEAR, lineClear_wav, sizeof(lineClear_wav));
    XGM_setPCM(SOUND_GAME_OVER, gameOver_wav, sizeof(gameOver_wav));
    XGM_setPCM(SOUND_START, start_wav, sizeof(start_wav));
    XGM_setPCM(SOUND_ARROW_MOVE, arrowMove_wav, sizeof(arrowMove_wav));
    XGM_setPCM(SOUND_ARROW_UP_DOWN, arrowUpDown_wav, sizeof(arrowUpDown_wav));
}
...
void OptionMoveLeft(bool loop)
{
    if (loop)
    {
        if (currOption->indexCol == 0)
        {
            ...
            XGM_startPlayPCM(SOUND_ARROW_MOVE, 10, SOUND_PCM_CH2);
        }
        else
        {
            currOption->indexCol--;
...
void OptionMoveUp(bool loop)
{
    if (loop)
    {
        if (currOption->indexRow == 0)
        {
            ...
            XGM_startPlayPCM(SOUND_ARROW_UP_DOWN, 10, SOUND_PCM_CH2);
        }
        else
        {
            currOption->anim = OPTION_UNSELECT;
