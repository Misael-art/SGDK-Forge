...
void handlestate(struct game *game){
	if(game->loaded_stage!=10 && game->current_stage==10) {
		...
		PAL_setColors(PAL0, (u16*)game_over.palette->data, 16, DMA);
		VDP_drawImageEx(BG_A, &game_over, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, ind), 0, 0, FALSE, TRUE);
		return;
	}
	if(game->loaded_stage!=0 && game->current_stage==0) {
        ...
        VDP_resetScreen();
        u16 ind=TILE_USER_INDEX;
        PAL_setColors(PAL0, (u16*)vara.palette->data, 16, DMA);
        VDP_drawImageEx(BG_A, &vara, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, ind), 0, 0, FALSE, TRUE);
    }
    if(game->loaded_stage!=1 && game->current_stage==1) {
        game->loaded_stage=1;
        VDP_resetScreen();
        u16 ind=TILE_USER_INDEX;
        PAL_setColors(PAL0, (u16*)titulo.palette->data, 16, DMA);
        VDP_drawImageEx(BG_A, &titulo, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, ind), 0, 0, FALSE, TRUE);
		SYS_disableInts();
		XGM_setPCM(SFX_START, start, sizeof(start));
		SYS_enableInts();
...
void run_stage(u16 current_stage, struct game *game) {
	...
	if(game->play_empty_sound){
		...
		XGM_startPlayPCM(SFX_FAIL,1,SOUND_PCM_CH2);
	}
...
void init_stage(u16 current_stage, struct game *game) {
	...
	u16 ind = TILE_USER_INDEX;
     SYS_disableInts();
    //init sounds
    XGM_setPCM(SFX_FAIL, fallo, sizeof(fallo));
	XGM_setPCM(SFX_HIT, hit, sizeof(hit));
    //init music
    ...
    // Load Background
    PAL_setColors(PAL0, (u16*)stage1.palette->data, 16, DMA);
    VDP_drawImageEx(BG_A, &stage1, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, ind), 0, 0, FALSE, TRUE);
    ...
    PAL_setPalette(PAL1,tiovara.palette->data, DMA);
    PAL_setPalette(PAL2,gente.palette->data, DMA);
    PAL_setPalette(PAL3,vararota.palette->data, DMA);
    for(i=0; i<PLAYERS_SIZE; i++) {
    	SPR_setAnim(game->players[i].player_sprite,ANIM_IDLE);
    }
}
