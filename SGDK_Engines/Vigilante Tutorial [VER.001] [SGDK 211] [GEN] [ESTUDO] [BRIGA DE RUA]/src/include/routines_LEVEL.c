...
void update_PLAYER_SPRITE()
{
    ...
    else if(player.state == STATE_JUMP_V)
    {
        ...
        if(player.counter_ANIM_V > 0)
        {
            ...
            player.pos_Y -= F32_toInt(G_GRAVITY);
    ...
    {
        /*fix32 total_grab = FIX32(G_NUMBER_GRAB_LEFT + G_NUMBER_GRAB_RIGHT + G_NUMBER_GRAB_2_LEFT + G_NUMBER_GRAB_2_RIGHT);
        
        VDP_drawIntEx_WINDOW_QUEUE_PRIO(F32_toInt(total_grab),1,0,0,PAL3);

        player.life -= F32_mul(FIX32(0.5),total_grab);*/
        ...
        if(player.pos_Y + F32_toInt(G_GRAVITY) < G_GROUND_POSITION)
        {
            G_GRAVITY += FIX32(0.5);

            SPR_setPosition(player.spr_PLAYER,player.pos_X,player.pos_Y += F32_toInt(G_GRAVITY));
        }
