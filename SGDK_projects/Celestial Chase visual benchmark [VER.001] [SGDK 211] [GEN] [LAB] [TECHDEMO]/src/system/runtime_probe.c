...
#define PROBE_SCENE_BOOTSTRAP_OFFSET 0x120
#define PROBE_SCENE_BOOTSTRAP_VERSION 1
#define PROBE_SCENE_BOOTSTRAP_LENGTH 12
#define PROBE_SCENE_BOOTSTRAP_HOLD_VERSION 2
#define PROBE_SCENE_BOOTSTRAP_HOLD_LENGTH 16
#define PROBE_SCENE_BOOTSTRAP_HOLD_FLAG 1
#define PROBE_SCENE_BOOTSTRAP_CHECKSUM_SEED 0xA55A
...
static u16 s_sceneCaptureTarget;
static u16 s_sceneCaptureHoldFrame;
static u16 s_sceneCaptureFlags;
static bool s_sceneCaptureHoldEnabled;
...
static void consume_scene_bootstrap(void)
{
    u32 offset = PROBE_SCENE_BOOTSTRAP_OFFSET;
    u16 version;
    u16 length;
    u16 sceneId;
    u16 holdFrame = 0;
    ...
    SRAM_enable();
    if (SRAM_readByte(offset + 0) != 'S'
        || SRAM_readByte(offset + 1) != 'B'
        || SRAM_readByte(offset + 2) != 'I'
        || SRAM_readByte(offset + 3) != 'S') {
        SRAM_disable();
        return;
    }

    version = sram_read_u16be(offset + 4);
    length = sram_read_u16be(offset + 6);
    sceneId = sram_read_u16be(offset + 8);
    if (version == PROBE_SCENE_BOOTSTRAP_HOLD_VERSION && length == PROBE_SCENE_BOOTSTRAP_HOLD_LENGTH) {
        holdFrame = sram_read_u16be(offset + 10);
        flags = sram_read_u16be(offset + 12);
        checksum = sram_read_u16be(offset + 14);
    } else {
        checksum = sram_read_u16be(offset + 10);
    }
    expected = PROBE_SCENE_BOOTSTRAP_CHECKSUM_SEED ^ version ^ length ^ sceneId ^ holdFrame ^ flags;

    SRAM_writeByte(offset + 0, 0);
    SRAM_writeByte(offset + 1, 0);
    SRAM_writeByte(offset + 2, 0);
    SRAM_writeByte(offset + 3, 0);
    SRAM_disable();

    if (!(
            (version == PROBE_SCENE_BOOTSTRAP_VERSION && length == PROBE_SCENE_BOOTSTRAP_LENGTH)
            || (version == PROBE_SCENE_BOOTSTRAP_HOLD_VERSION && length == PROBE_SCENE_BOOTSTRAP_HOLD_LENGTH)
        )
        || checksum != expected
        || sceneId > APP_SCENE_CHASE) {
        return;
    }

    s_sceneCaptureTarget = sceneId;
    s_sceneCaptureHoldFrame = holdFrame;
    s_sceneCaptureFlags = flags;
    s_sceneCaptureHoldEnabled = (
        version == PROBE_SCENE_BOOTSTRAP_HOLD_VERSION
        && (flags & PROBE_SCENE_BOOTSTRAP_HOLD_FLAG) != 0
    );

    gApp.currentScene = (AppScene)sceneId;
    gApp.previousScene = (AppScene)sceneId;
    gApp.transitionTarget = (AppScene)sceneId;
    gApp.sceneFrames = 0;
...
void MDRuntimeProbe_init(void)
{
    ...
    s_sceneCaptureTarget = 0;
    s_sceneCaptureHoldFrame = 0;
    s_sceneCaptureFlags = 0;
    s_sceneCaptureHoldEnabled = FALSE;
    consume_scene_bootstrap();
...
{
    return s_sceneCaptureHoldEnabled
        && (u16)gApp.currentScene == s_sceneCaptureTarget
        && gApp.sceneFrames >= s_sceneCaptureHoldFrame;
}

bool MDRuntimeProbe_consumeChaseFailureBootstrap(void)
{
    if ((s_sceneCaptureFlags & MD_RUNTIME_PROBE_BOOTSTRAP_FORCE_CHASE_FAILURE) == 0) {
        return FALSE;
    }

    s_sceneCaptureFlags &= (u16)(~MD_RUNTIME_PROBE_BOOTSTRAP_FORCE_CHASE_FAILURE);
    return TRUE;
}
