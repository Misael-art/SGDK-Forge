...
/*
 * Number of valid AppScene ids. Defined defensively so that if game_vars.h
 * later grows a canonical APP_SCENE_COUNT this file picks it up instead.
 */
#ifndef APP_SCENE_COUNT
#define APP_SCENE_COUNT 12
#endif
...
const char* APP_sceneName(AppScene scene)
{
    switch (scene)
    {
        ...
        case APP_SCENE_NATIVE_ART_REVIEW: return "ARTREVIEW";
...
void APP_update(void)
{
    ...
    if (gApp.sceneNeedsEnter)
    {
        switch (gApp.currentScene)
        {
            ...
            case APP_SCENE_NATIVE_ART_REVIEW: SCENE_nativeArtReviewEnter(); break;
    ...
    switch (gApp.currentScene)
    {
        ...
        case APP_SCENE_NATIVE_ART_REVIEW: SCENE_nativeArtReviewUpdate(); break;
