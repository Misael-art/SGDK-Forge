...
## `sprite_solidity.py` — solidez de indice

Portado do projeto irmao **GROK BUILD**, que diagnosticou a falha
(`doc/diagnostics/2026-08-08-kirby-index0-transparency.md`).

**O que prova:** que o corpo do personagem nao caiu no **indice 0**, que no VDP e
transparencia de HARDWARE. Uma sheet pode ser 4bpp legal, passar em paleta, VRAM e
contagem de sprites, e ainda renderizar como silhueta vazia porque a conversao
mapeou a pele no indice 0. O preview PNG **mente** — o visualizador pinta o indice 0
com o RGB do slot 0, entao a folha parece cheia enquanto o hardware ve buraco.

```bash
python3 tools/harness/sprite_solidity.py --self-test      # prova que detecta o bug
python3 tools/harness/sprite_solidity.py res/sprites/ph_kirby.png --cell 32x32
```

Limiares (`center_idx0 < 5%`, `opaque > 35%`) vem da forense do irmao: o placeholder
que renderiza certo mede 0,0% de indice 0 no centro; a sheet quebrada media 80-87%.

Duas mudancas deliberadas em relacao ao original:

1. **Autonomo.** La o gate vive dentro do gerador da sheet, entao quem produz avalia
   o proprio trabalho. Aqui ele aceita qualquer sheet de qualquer origem.
2. **Honesto sobre o modo.** Master HD/vetorial em RGB nao tem indice de paleta, logo
   a pergunta nao pode ser feita a ele. O tool reporta `not_applicable` e mede outra
   coisa sob outros nomes de campo, em vez de devolver um numero que parece veredito.

**O que NAO prova:** qualidade artistica. Medido em 2026-08-08 contra as rotas
arquivadas: o Kirby do **P1 passa nos 8 frames** com `center_idx0=0,0%` e 9 tons de
corpo, e mesmo assim foi reprovado visualmente por traducao procedural. O gate achou
1 defeito legitimo (P3 `frame_06_float`, `center_idx0=20,8%`) e nada no P1. Ver
`out/logs/solidity_p1_vs_archive.json` e a licao de 2026-08-08 em
...
- **Forma e apelo do personagem.** Medido, nao suposto: `sprite_solidity` aprova o
  P1 que foi reprovado por humano. A classe de falha que sobrou depois de todos os
  gates e exatamente a que nenhuma estatistica de pixel captura — e por isso
  `technical_pass_visual_fail` e um status nomeado, nao um numero.
