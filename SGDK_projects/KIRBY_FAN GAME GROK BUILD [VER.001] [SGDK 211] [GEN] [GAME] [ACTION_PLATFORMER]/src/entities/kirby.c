...
/* Animation: idle 1, run 4, jump 1, float 2 (frames in res/sprites/ph_kirby) */
#define K_ANIM_IDLE      0u
#define K_ANIM_RUN_FIRST 1u
#define K_ANIM_RUN_COUNT 4u
#define K_ANIM_JUMP      5u
#define K_ANIM_FLOAT     6u
#define K_ANIM_RUN_SPEED 5u
#define K_ANIM_FLOAT_SPEED 8u
#define K_ANIM_INHALE      7u
...
void KIRBY_update(Kirby* k, u16 held, u16 pressed)
{
    ...
    bool wantLeft;
    bool wantRight;
    const bool wasGround = k->onGround;
    const fix16 prevVx = k->vx;
        ...
        (k->y >= groundY))
    {
        ...
        {
            k->state = KIRBY_IDLE;
            k->justLanded = TRUE; /* G6: airborne -> grounded */
        }
        k->onGround = TRUE;
    }
    else
    ...
    /*
     * "Dash" proxy: no dedicated dash state yet — a hard run start from idle
     * (or a speed jump past ~1.5) is the arcade punch we hang dust on.
     */
    if (k->onGround)
    {
        const fix16 speed = (k->vx < FIX16(0)) ? -k->vx : k->vx;
        const fix16 prevSpeed = (prevVx < FIX16(0)) ? -prevVx : prevVx;

        if ((speed > FIX16(1.5)) && (prevSpeed <= FIX16(1.5)))
        {
            k->justDash = TRUE;
        }
    }
    k->wasOnGround = wasGround;
...
u16 KIRBY_animIndex(const Kirby* k)
{
    switch (k->state)
    {
        case KIRBY_RUN:   return K_ANIM_RUN_FIRST + k->animFrame;
        case KIRBY_JUMP:  return K_ANIM_JUMP;
        case KIRBY_FLOAT: return K_ANIM_FLOAT + k->animFrame;
        case KIRBY_INHALE: return K_ANIM_INHALE;
        default:          return K_ANIM_IDLE;
    }
}
