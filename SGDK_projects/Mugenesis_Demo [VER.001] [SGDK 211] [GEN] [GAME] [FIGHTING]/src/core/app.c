...
#ifndef MG_DIRECT_FIGHT
#define MG_DIRECT_FIGHT 1   /* fatia vertical: inicia direto na luta (evidencia de emulador) */
#endif
    gApp.currentScene = MG_DIRECT_FIGHT ? APP_SCENE_DEMO : APP_SCENE_BRANDING;
...
const char* APP_sceneName(AppScene scene)
{
    switch (scene)
    {
        ...
        case APP_SCENE_DEMO: return "FIGHT";
...
{
    /* C e botao de chute na luta: o toggle do HUD de debug fica fora da cena de luta */
    if (gApp.currentScene != APP_SCENE_DEMO && INPUT_pressed(BUTTON_C)) {
