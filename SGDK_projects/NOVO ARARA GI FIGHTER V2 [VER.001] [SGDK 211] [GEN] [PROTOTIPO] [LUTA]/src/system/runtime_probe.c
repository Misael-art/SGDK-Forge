...
void MDRuntimeProbe_tick(void)
{
    ...
    /*
     * SPR_getUsedVDPSprite() is a total SAT count, not a per-scanline count.
     * The runtime metrics schema records word[14] as scanline pressure, so
     * use a conservative proxy for 96x112 SGDK fighter sprites: they are split
     * across roughly four 32px-high VDP sprite rows, while word[16] keeps the
     * active high-level sprite count.
     */
    u16 scanlineSpritePressure = (u16)((usedVdpSprites + 3u) / 4u);
    ...
    if (scanlineSpritePressure > 20) scanlineSpritePressure = 20;
    if (scanlineSpritePressure > g_mdRuntimeProbe[14]) {
        g_mdRuntimeProbe[14] = scanlineSpritePressure;
    }
    ...
    samplesRecorded = g_mdRuntimeProbe[9];
    if ((samplesRecorded >= MD_RUNTIME_PROBE_MAX_SAMPLES) &&
        (samplesRecorded != s_lastExportSamples) &&
        ((gApp.totalFrames - s_lastExportFrame) >= 60u)) {
