...
#define APP_SCENE_BOOTSTRAP_VERSION 1u
#define APP_SCENE_BOOTSTRAP_SIZE 12u

static u16 appReadSramU16(u32 offset)
{
    return (u16)(((u16)SRAM_readByte(offset) << 8) | SRAM_readByte(offset + 1u));
}

static bool appReadSceneBootstrap(AppScene *scene)
{
    const u16 version = appReadSramU16(APP_SCENE_BOOTSTRAP_OFFSET + 4u);
    const u16 payloadSize = appReadSramU16(APP_SCENE_BOOTSTRAP_OFFSET + 6u);
    const u16 sceneId = appReadSramU16(APP_SCENE_BOOTSTRAP_OFFSET + 8u);
    const u16 checksum = appReadSramU16(APP_SCENE_BOOTSTRAP_OFFSET + 10u);
    const u16 expected = (u16)(0xA55Au ^ version ^ payloadSize ^ sceneId);
    ...
    if (SRAM_readByte(APP_SCENE_BOOTSTRAP_OFFSET + 0u) != (u8)'S'
        ...
        || version != APP_SCENE_BOOTSTRAP_VERSION
        || payloadSize != APP_SCENE_BOOTSTRAP_SIZE
        || checksum != expected
...
void APP_boot(bool hardReset)
{
    ...
    INPUT_init();
    AUDIO_init();
    SPR_initEx(64);   /* D1 envelope: two 32x40 fighters plus a measured FX window */
    ...
    SRAM_enableRO();
    {
        AppScene bootstrapScene;
        if (appReadSceneBootstrap(&bootstrapScene)) {
            gApp.currentScene = bootstrapScene;
            gApp.previousScene = bootstrapScene;
            gApp.transitionTarget = bootstrapScene;
        }
    }
    SRAM_disable();
