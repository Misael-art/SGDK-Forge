...
#define AUDIO_BGM_BRAND 1u
#define AUDIO_BGM_FIGHT 2u
...
{
    sBgmWanted = 0;
    /* Teardown must invalidate the pending PSG cue before the next scene
     * becomes owner. Otherwise AUDIO_update() can re-arm a stale envelope
     * after a branding/menu -> combat transition. */
    sCueFrames = 0;
    audioStopPsg();
...
{
    sBgmWanted = AUDIO_BGM_BRAND;
    XGM2_play(mus_forge_brand);
}

void AUDIO_startFightBgm(void)
{
    sBgmWanted = AUDIO_BGM_FIGHT;
    XGM2_play(mus_cinder_circuit_arena);
}
