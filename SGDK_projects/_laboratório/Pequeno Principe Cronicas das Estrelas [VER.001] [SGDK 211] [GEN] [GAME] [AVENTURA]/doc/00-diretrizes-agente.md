...
| 4 | Criar cena, planeta ou mecanica nao prevista no GDD | Scope creep |
...
| H-Int | 1 callback por frame | Reconfigurar e caro; centralizar em `hint_manager` |
...
- [ ] O H-Int continua centralizado em `hint_manager.c`?
...
2. A ROM roda no emulador (Gens/Blastem/Kega) sem crash visivel.
...
5. `doc/10-memory-bank.md` foi atualizado com o que mudou.
6. Nenhum documento canonico ficou em conflito com o estado real.
...
1. Esta criando efeito visual sem saber o budget de DMA da cena.
2. Esta escrevendo dialogo sem consultar o roteiro.
3. Esta adicionando mecanica que nao existe no GDD.
4. Esta usando sinFix16/cosFix16 sem entender o custo no VBlank.
5. Esta modificando `project.h` sem saber o impacto em todos os modulos.
6. Nao consegue compilar depois da mudanca.
7. Esta criando um segundo sistema de scroll, state machine ou render.
