...
void stateCharSelect() {
    ...
    while (currentState == STATE_CHAR_SELECT) {
        ...
        char textBuf[64];
        sprintf(textBuf, "Cursor Selecao: %s    ", charNames[cursorChar]);
        VDP_drawText(textBuf, 2, 6);
        
        sprintf(textBuf, "P1 Selecionado: %s    ", p1Char == -1 ? "NENHUM" : charNames[p1Char]);
        VDP_drawText(textBuf, 2, 8);
        
        sprintf(textBuf, "P2 Selecionado: %s    ", p2Char == -1 ? "NENHUM" : charNames[p2Char]);
...
void stateAnimTest() {
    ...
    sprP2 = SPR_addSprite(def2, 180, 100, TILE_ATTR(PAL2, TRUE, FALSE, TRUE)); // P2 invertido horizontalmente
