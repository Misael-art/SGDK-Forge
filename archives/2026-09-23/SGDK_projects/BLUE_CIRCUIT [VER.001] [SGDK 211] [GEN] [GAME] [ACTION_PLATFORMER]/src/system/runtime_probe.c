...
#define PROBE_CAPTURE_FRAMES_NTSC 900
#define PROBE_CAPTURE_FRAMES_PAL 750
...
#define PROBE_VISUAL_SRAM_SCHEMA_VERSION 1
#define PROBE_VISUAL_METRIC_WORDS 24
#define PROBE_VISUAL_PALETTE_WORDS 64
#define PROBE_VISUAL_SRAM_TOTAL_BYTES (8 + ((PROBE_VISUAL_METRIC_WORDS + PROBE_VISUAL_PALETTE_WORDS) * 2))
...
static bool s_captureExported;
static u16 s_vlabPalette[PROBE_VISUAL_PALETTE_WORDS];
...
static void reset_scene_metrics(u16 sceneId, u16 cpuLoad)
{
    ...
    g_mdRuntimeProbe[21] = SYS_isPAL() ? PROBE_CAPTURE_FRAMES_PAL : PROBE_CAPTURE_FRAMES_NTSC;
...
void MDRuntimeProbe_exportVisualToSRAM(void)
{
    u32 offset = MD_RUNTIME_PROBE_VISUAL_OFFSET;
    u32 frame = gApp.totalFrames;
    u16 i;

    PAL_getColors(0, s_vlabPalette, PROBE_VISUAL_PALETTE_WORDS);

    SRAM_enable();

    SRAM_writeByte(offset + 0, (u8) 'V');
    SRAM_writeByte(offset + 1, (u8) 'L');
    SRAM_writeByte(offset + 2, (u8) 'A');
    SRAM_writeByte(offset + 3, (u8) 'B');
    sram_write_u16be(offset + 4, PROBE_VISUAL_SRAM_SCHEMA_VERSION);
    sram_write_u16be(offset + 6, PROBE_VISUAL_SRAM_TOTAL_BYTES);
    ...
    for (i = 0; i < PROBE_VISUAL_PALETTE_WORDS; i++) {
...
void MDRuntimeProbe_tick(void)
{
     ...
     */
    if (!s_captureExported && (g_mdRuntimeProbe[8] % MD_RUNTIME_PROBE_HEARTBEAT_PERIOD) == 0u) {
    ...
    dmaQueueEntries = DMA_getQueueSize();
    dmaQueueBytes = DMA_getQueueTransferSize();
    if (dmaQueueEntries > g_mdRuntimeProbe[18]) {
        g_mdRuntimeProbe[18] = dmaQueueEntries;
    }
    if (dmaQueueBytes > g_mdRuntimeProbe[19]) {
        g_mdRuntimeProbe[19] = dmaQueueBytes;
    }

    captureTarget = g_mdRuntimeProbe[21];
    if (captureTarget > MD_RUNTIME_PROBE_MAX_SAMPLES) {
        captureTarget = MD_RUNTIME_PROBE_MAX_SAMPLES;
    }

    if (samplesRecorded < captureTarget) {
    ...
    samplesRecorded = g_mdRuntimeProbe[9];
    if (!s_captureExported && captureTarget > 0 && samplesRecorded >= captureTarget) {
        g_mdRuntimeProbe[22] = 1;
        MDRuntimeProbe_exportToSRAM();
        MDRuntimeProbe_exportVisualToSRAM();
        s_captureExported = TRUE;
    }
}
