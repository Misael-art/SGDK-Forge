...
- title screen, logo do jogo, press-start, menu principal ou front-end com identidade de produto tambem devem referenciar `brand_identity_manifest`
...
- `brand_identity_manifest` decide fonte-display, logo, fonte-body de front-end e fallback; o `ui_decision_card` continua dono da surface e do budget
...
- UI pixel art que use free-scale, subpixel motion, AA, blur ou interpolacao
- health bar final sem container, buffer de dano latente, fill ativo e drenagem por pixels inteiros quando a leitura de dano for mecanica relevante
- logo/title/menu final sem `brand_identity_manifest` quando houver identidade de produto
- logo que falha em miniatura, monocromatico, silhueta ou fundo dinamico
...
Anexo de identidade de marca condicional:

- obrigatorio quando a surface for title screen, logo, press-start, menu principal ou front-end com identidade de produto
- schema: `tools/sgdk_wrapper/schemas/brand_identity_manifest.schema.json`
- campos cobertos pelo manifesto:
  - `logo_system`: nome principal, subtitulo, peso visual, metafora de gameplay, testes de silhueta/monocromatico/miniatura/fundo dinamico
  - `typography_system`: fonte-display, fonte-body, fonte-HUD/narrativa quando aplicavel, charset PT-BR, politica anti-fonte-generica e `glyph_manifest_ref`
  - `title_screen_export_plan`: camadas runtime do logo, superficie VDP, dominio de paleta, animacao e fallback estatico
  - `validation_plan`: leitura 320x224, testes de escala/contraste e evidencia BlastEm quando aprovado para runtime
- o manifesto nao substitui `ui_decision_card`; ele impede que a identidade visual do produto vire fonte generica ou logo decorativo sem prova

Anexo pixel-perfect condicional:

- obrigatorio quando a surface tiver health bar, caixa de texto, micro-icones,
  cursor, fonte ou atlas de UI que precisa sobreviver como pixel art nativa
- schema: `tools/sgdk_wrapper/schemas/ui_pixel_surface_contract.schema.json`
- campos cobertos pelo contrato:
  - `pixel_grid_policy`: resolucao nativa, posicionamento inteiro, politica de
    escala e interpolacoes proibidas;
  - `typography_policy`: fonte, altura nativa, baseline, tratamento de
    contraste e `glyph_manifest`;
  - `atlas_plan`: atlas, surface VDP, dominio de paleta, politica de index 0 e
    budget;
  - `health_bar_system`: container, buffer de dano latente, fill ativo,
    drenagem por pixels inteiros, threshold critico e feedback de low HP;
  - `nine_slice_system`: cantos estaticos, bordas repetidas em um eixo e centro
    repetido ou preenchido;
  - `validation_plan`: leitura nativa, movimento inteiro, budget de atlas e
    evidencia BlastEm quando aprovado para runtime.

Regras de health bar:

- a barra de vida nao e um retangulo colorido; e um sistema de container,
  buffer de dano latente e fill ativo;
- o fill deve drenar em passos inteiros de 1 ou 2 pixels;
- a borda interna do fill deve ser hard-edge, sem AA;
- o estado critico deve ter threshold declarado, flash/cadence controlado e
  opcionalmente audio cue;
- se o jogo depende de leitura de impacto, health bar sem buffer latente fica
  no maximo `needs_review`.

O contrato pixel-perfect nao substitui `ui_decision_card`; ele prova que a
surface escolhida respeita a grade, a fonte, o atlas e a leitura em runtime.

Anexo de apresentacao expressiva de texto:

- obrigatorio quando texto, fala ou alerta tiver peso dramatico, cinetico ou diegetico
- doutrina: `doc/03_art/16_expressive_narrative_text_presentation.md`
- campos:
  - `text_surface_class`
  - `presentation_role`
  - `reading_context`
  - `narrative_timing_model`
  - `layout_anchor`
  - `speaker_binding`
  - `text_audio_plan`
  - `asset_budget_plan`
  - `teardown_reset_plan`
- o anexo nunca substitui `glyph_manifest`; ele decide encenacao, ritmo, ownership e fallback
...
- `text_surface_class`
  - valores canonicos: `panel_sequence_text`, `diegetic_speech_balloon`, `animated_portrait_dialog`, `kinetic_hype_text`, `typewriter_voice_text`, `flavor_text_interaction`
...
brand_identity_manifest_ref: "doc/brand_identity_manifest.json"
ui_pixel_surface_contract_ref: "doc/ui_pixel_surface_contract.json"
...
text_presentation_profile:
  text_surface_class: kinetic_hype_text
  presentation_role: "entrada curta de impacto sem bloquear navegacao"
  reading_context: "menu"
  narrative_timing_model: "timed_burst"
  layout_anchor: "WINDOW"
  speaker_binding: "none"
  text_audio_plan: "stinger_ref"
  asset_budget_plan: "sem cache temporario; usa display_font ja residente"
  teardown_reset_plan: "limpar WINDOW, paleta de pulso e stinger curto ao sair"
  fallback_plan: "titulo fixo com pulso de cursor"
```

## Handoff entre skills

- `visual-excellence-standards`
  - decide linguagem visual, legibilidade, `attention_profile`, `hud_density` e `ui_pixel_surface_contract` quando a UI depender de pixel-perfect
- `art-direction-selector`
  - emite ou corrige `brand_identity_manifest` junto do `master_style_manifest` quando o projeto tiver logo/title/front-end autoral
...
- `art-translation-to-vdp`
  ...
  - entra quando paineis, retratos ou baloes exigirem arte dedicada de VDP
...
- health bar de acao deve ter `ui_pixel_surface_contract`; sem container + buffer latente + fill inteiro, fica `needs_review`
...
- texto narrativo com encenacao deve declarar `text_presentation_profile`
- balao, painel, retrato animado, hype text ou typewriter voice sem owner e fallback deve ser recusado
...
- menu/title/logo de entrega deve ter `brand_identity_manifest`; sem ele, a identidade fica `needs_review`
