...
| Status | Itens |
|---|---|
| `ja_canonizado` | HUD/UI por `ui_decision_card`, tipografia hibrida com `glyph_manifest`, apresentacao expressiva de texto por `text_presentation_profile`, menu/title como front-end formal, transicoes por `scene_transition_card`, raster/luz/feedback FX, boss/setpieces, tilemap avancado, audio senior, multi-plano basico, budget VDP basico, DMA safety e tile flipping |
...
| S3.5 | Expressive Narrative Text Presentation | `INCORPORADO` | `NAO_INICIADA` | Sprint 2 canonizada | doutrina formal em `doc/03_art/16_expressive_narrative_text_presentation.md`; texto dramatico entra como `text_presentation_profile` dentro do `ui_decision_card`, nao como artefato paralelo |
...
| 2026-04-22 | S3.5 | `PENDENTE -> INCORPORADO` | `Expressive Narrative Text Presentation` canoniza paineis, baloes, retratos animados, hype text, typewriter voice e flavor text como anexo objetivo do `ui_decision_card` |
