...
## 0. Escopo de Composicao — Contrato de Producao

Toda trilha, loop ou ambience deve declarar um `composition_scope_contract` antes de ser produzido ou integrado.
O objetivo e impedir que um rascunho rapido seja tratado como musica final e garantir que o esforco de composicao
corresponda ao papel real da cena.
Quando materializado em JSON, o contrato deve validar contra
`tools/sgdk_wrapper/schemas/composition_scope_contract.schema.json`.

| Escopo canonico | Uso permitido | Saida minima | Bloqueio |
|-----------------|---------------|--------------|----------|
| `micro_sketch_1m` | laboratorio, placeholder, loading, menu simples, game jam, textura provisoria | loop de 2-4 compassos ou motivo imediato, identidade sonora anotada, status ceiling declarado | nao fecha entrega AAA nem substitui faixa de fase sem promocao documentada |
| `core_loop_10m` | fase generica, exploracao, menu principal simples, vertical slice funcional | loop de 8-16 compassos ou duracao equivalente, `seamless_loop_report`, ownership de canais e prioridade de SFX | bloqueia se clicar, cortar, cansar rapidamente ou mascarar feedback critico |
| `modular_track_1h` | cena AAA, boss, set piece, fase principal, musica adaptativa | estrutura A/B, intro/ponte/stinger ou variacao formal, stems/layers quando aplicavel, `frequency_masking_plan`, `adaptive_music_state_map` | bloqueia se for arquivo achatado sem plano de camadas, transicao, fallback e protecao de SFX |
| `silence_intentional` | cena sem musica por decisao dramatica ou tecnica | justificativa, eventos de SFX/ambience que ocupam o papel sonoro, fallback | bloqueia se for ausencia acidental de audio |

Gates praticos:

- BGM final AAA nunca pode nascer de `micro_sketch_1m` sem nova passagem por `core_loop_10m` ou `modular_track_1h`.
- Todo loop musical precisa de `seamless_loop_report` com ponto de retorno, ausencia de clique e comportamento em pause/resume.
- Toda musica que divide tela com SFX de gameplay precisa de `frequency_masking_plan`: hit, dano, alerta, passos, tiros, UI, voz e boss cue ficam acima de textura musical.
- Trilha adaptativa exige stems/layers mapeados para estados de gameplay; do contrario e BGM linear, nao score modular.

## 0.1 Identidade Sonora — Mega Drive nao e SNES Rebaixado

Quando a referencia musical vier de SNES, arcade sampleado, orquestral ou cinema, o agente deve traduzir a intencao para a escola sonora do Mega Drive:

- `YM2612`: baixo, lead, pad, arp, percussao sintetica e stinger nascem de patch/timbre, envelope e ownership de canal.
- `PSG`: UI, noise, acentos, drones e fallback barato; nao e canal residual sem papel.
- `DAC/PCM`: voz, impacto ou bateria curta; sempre com tradeoff de canal, ROM, sample audit e risco de barramento.
- `Eco/reverb SNES`: nao existe como efeito nativo; se for desejado, precisa ser simulado por composicao, delay musical, PSG/FM layering ou decisao de sample com budget.

Por isso todo `audio_architecture_card` de cena com identidade sonora deve conter `sound_chip_identity_plan`. Ele responde: qual escola sonora o projeto escolhe, quais papeis ficam em FM/PSG/DAC, que referencia esta sendo traduzida e quais suposicoes ficam proibidas.
