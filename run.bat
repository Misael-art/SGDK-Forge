@echo off
REM =========================================================================
REM run.bat - Delegacao canonica para tools\\sgdk_wrapper
...
if exist "%~dp0tools\sgdk_wrapper\run.bat" if exist "%~dp0tools\sgdk_wrapper\prepare_assets.py" for %%I in ("%~dp0tools\sgdk_wrapper") do set "SGDK_WRAPPER_ROOT=%%~fI"
...
if not defined SGDK_WRAPPER_ROOT if exist "%~dp0..\..\tools\sgdk_wrapper\run.bat" for %%I in ("%~dp0..\..\tools\sgdk_wrapper") do set "SGDK_WRAPPER_ROOT=%%~fI"
if not defined SGDK_WRAPPER_ROOT if exist "%~dp0..\..\..\tools\sgdk_wrapper\run.bat" for %%I in ("%~dp0..\..\..\tools\sgdk_wrapper") do set "SGDK_WRAPPER_ROOT=%%~fI"
if not defined SGDK_WRAPPER_ROOT (
    echo [ERROR] Nao foi possivel localizar tools\sgdk_wrapper a partir de %~dp0
    endlocal & exit /b 1
)
call "%SGDK_WRAPPER_ROOT%\\run.bat" "%SGDK_PROJECT_ROOT%"
