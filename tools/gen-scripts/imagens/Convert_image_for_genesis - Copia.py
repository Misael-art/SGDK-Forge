...
def add_padding(imgs, size):
  ...
  for imgl in imgs:
    for img in imgl:
        ...
        #print(img.shape)
        imgs2.append(cv2.copyMakeBorder(img.copy(), (size- img.shape[0])//2,
                                        (size- img.shape[0])//2, 
                                        (size- img.shape[1])//2,
                                        (size- img.shape[1])//2,
...
numero_de_cores = 16
limitar_cores = True
arquivo_de_saida = list_images_from_folder(ROOT)+'output.png'

# aplicação de filtro
if autodetect_backgroud_color:
  background_color = imgs[0][0][0]
if tamanho_padding > 0:
  #print(imgs, type(imgs), 'imgs') 
  #print(tamanho_padding, type(tamanho_padding), 'tamanho_padding')   
  imgs = add_padding(imgs, tamanho_padding)
