...
def analyze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for frame_idx in range(frame_count):
        ...
        if component_report["small_island_count"] >= args.max_small_islands or component_report["small_island_pixels"] >= args.max_small_island_pixels:
            ...
            add_finding(
                ...
                frame=frame_number,
                small_island_count=component_report["small_island_count"],
                small_island_pixels=component_report["small_island_pixels"],
            )
        stray_pixels = int(component_report["non_largest_component_pixels"])
        largest_stray = int(component_report["largest_stray_component_area"])
        stray_ratio = stray_pixels / max(1, len(xs))
        if largest_stray >= args.max_stray_component_pixels or stray_ratio >= args.max_stray_component_ratio:
            island_problem_frames += 1
            add_finding(
                findings,
                "STRAY_LARGE_COMPONENT",
                ...
                largest_stray_component_box=component_report["largest_stray_component_box"],
        ...
        if args.detect_baked_fx and fx_pixels >= args.fx_pixel_threshold:
            baked_fx_frames += 1
            add_finding(
                findings,
                "BAKED_FX_IN_CHARACTER_SHEET",
    ...
    if bbox_heights:
        ...
        if ratio > args.max_scale_ratio:
            add_finding(
                ...
                max_height=max_h,
                ratio=ratio,
                max_allowed=args.max_scale_ratio,
            )
...
def main() -> int:
    ...
    parser.add_argument("--max-scale-ratio", type=float, default=1.18)
