...
- `camera_behavior_seed` quando genero, velocidade, plataforma, chase, boss room ou escala afetarem visibilidade e resposta da camera
...
- `brand_identity_seed` quando houver logo, title screen, press-start, menu principal ou front-end autoral
- `character_scale_seed` quando personagem, inimigo, lutador, boss ou avatar afetar FOV, hitbox, camera ou carga de animacao
- `camera_behavior_seed` quando a primeira cena exigir deadzone, look-ahead, clamp, trigger, fixed room, auto-scroll ou shake
- `ui_pixel_surface_seed` quando HUD/UI incluir health bar, fonte, caixa, micro-icone, cursor ou atlas pixel-perfect
- `technical_visual_audio_ambition`
...
- `project_brief`, `core_loop_statement`, `feature_scope_map`, `front_end_profile`, `creative_director_radar_seed`, `brand_identity_seed`, `character_scale_seed`, `camera_behavior_seed`, `ui_pixel_surface_seed`, `technical_visual_audio_ambition` e `technique_selection_seed` -> `doc/11-gdd.md`
...
- camera do slice ja declara se usa cena fixa, deadzone, look-ahead, clamp, trigger, auto-scroll ou shake, e qual problema de gameplay ela resolve
...
- `camera_behavior_seed` quando houver camera jogavel ou scroll com impacto mecanico
