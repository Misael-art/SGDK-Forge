...
Depois de classificar `projeto_existente`, `reseed` ou `projeto_novo`, execute
obrigatoriamente `workflows/project-methodology-adoption.md`. Projeto antigo
sem manifesto recebe materializacao segura, mas nenhum claim e inferido por
regex ou palavra solta.

`doc/project_context_manifest.json` deve estar presente e nao pode permanecer
`unclassified` quando a sessao entrar em producao, review final ou consultoria.
O report `out/logs/project_context_report.json` informa quais documentos sao
realmente bloqueantes para o contexto selecionado.

Antes de ler ou gerar material operacional, valide tambem `doc/project_hygiene_manifest.json`.
Todo material especifico do projeto deve estar dentro dele; entradas externas usadas
devem ter copia local organizada em `rascunho/` e hash registrado.
Codigo, scripts, manifestos e documentacao ativa com caminho absoluto para outro
workspace/projeto bloqueiam a abertura ate apontarem para a copia local.
Material ativo precisa cumprir `naming_policy=portable_descriptive_v1`; nomes
legados fora do perfil exigem plano de migracao, nunca renomeacao cega.
...
- materializar/classificar `doc/project_methodology_manifest.json` sem sobrescrever decisoes locais
- executar `audit_project_learning.ps1 -Mode Audit -OutputFormat Json`, classificar `doc/agent_learning/` sem escrever e carregar primeiro apenas o `candidate_index`
...
- materializar/classificar `doc/project_methodology_manifest.json` com `lifecycle=reseed`
...
- confirmar o manifesto metodologico do template com `lifecycle=new`
...
- tecnicas escolhidas/rejeitadas com registry IDs, funcao, owner, budget e fallback
...
Aprendizado observado durante a execucao pode atualizar automaticamente o ledger local, mas nunca vira canon automaticamente.

Fluxo correto:

1. abrir com `audit_project_learning.ps1 -Mode Audit`, sem escrita;
2. registrar observacao real nos arquivos locais;
3. validar em build/evidencia quando aplicavel;
4. fechar com `audit_project_learning.ps1 -Mode Capture`;
5. deduplicar contra owner existente e gerar proposta `not_applied`;
6. obter aprovacao humana quando a curadoria alterar comportamento canonico;
7. so entao promover por patch controlado para workflow, skill, regra, validator, schema, `lib_case` ou memoria canonica de framework.

O local padrao e `doc/agent_learning/`, seguindo `workflows/project-learning-loop.md`. O ciclo automatico deve encerrar com `canonical_promotion_performed=false`.
...
- o agente classificou corretamente `aaa_game`, `technical_demo`, `exercise`, `game_review` ou `consulting`
- `project_context_report.json` nao possui blocker de contexto
- `project_methodology_report.json` nao possui claim `review_required`
...
- `workflows/project-learning-loop.md`
  - quando houver `doc/agent_learning/` presente, ausente ou incompleto a classificar
