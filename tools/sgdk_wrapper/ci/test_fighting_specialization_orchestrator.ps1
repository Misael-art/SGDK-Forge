<#
.SYNOPSIS
    Verifies the fighting-game-design orchestrator skill is a thin
...
$skillRoot = Join-Path $wrapperRoot '.agent\skills\planning\fighting-game-design'
$skillMd = Join-Path $skillRoot 'SKILL.md'
$openaiYaml = Join-Path $skillRoot 'agents\openai.yaml'
$lexicon = Join-Path $skillRoot 'references\fighting_design_lexicon.md'
$orchestratorMap = Join-Path $skillRoot 'references\fighting_orchestrator_map.md'
...
Write-Host ''
Write-Host '=== Fighting Specialization Orchestrator Test ==='
Write-Host ''
...
Assert-True 'references/fighting_design_lexicon.md present' (Test-Path -LiteralPath $lexicon) $lexicon
Assert-True 'references/fighting_orchestrator_map.md present' (Test-Path -LiteralPath $orchestratorMap) $orchestratorMap
...
Assert-True 'default_prompt mentions fighting_2d_traditional' ($yaml -match 'fighting_2d_traditional') ('not found')
Assert-True 'default_prompt forbids folder-name inference' ($yaml -match 'NUNCA infere') ('not found')
Assert-True 'default_prompt references canonical validator' ($yaml -match 'validate_fighting_specialization\.ps1') ('not found')
...
Assert-True 'SKILL.md is thin (lines <= 180)' ($skillLineCount -le 180) ("$skillLineCount lines")
Assert-True 'SKILL.md references all 5 schemas' (($skillContent -match 'genre_specialization_registry\.schema\.json') -and ($skillContent -match 'genre_specialization_manifest\.schema\.json') -and ($skillContent -match 'fighting_2d_design_contract\.schema\.json') -and ($skillContent -match 'fighting_moveset_frame_data\.schema\.json') -and ($skillContent -match 'fighting_specialization_report\.schema\.json')) ('missing schema reference')
...
Assert-True 'SKILL.md pins rollback_netcode=not_applicable' ($skillContent -match 'rollback_netcode\s*=\s*"not_applicable"') ('not explicit')
