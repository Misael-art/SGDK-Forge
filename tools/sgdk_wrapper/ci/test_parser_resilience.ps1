...
function Assert-Case {
    ...
    try {
        ...
        $lastExitCode = if ($lastExitCodeVar) { [int]$lastExitCodeVar.Value } else { 0 }
    ...
    if ($showVerbose -and $stderr) { Write-Host "   stderr: $stderr" }
...
try {
    ...
    # Case 1b: valido com MDRT no offset 0 (fallback auto-detectado)
    $p1b = Join-Path $tempRoot 'c1b_offset_zero.sram'
    $zeroOffset = New-SyntheticMdrt -WordCount 64 -SamplesRecorded 0
    $offsetZeroBytes = New-Object byte[] ($zeroOffset.Length - 0x200)
    [Array]::Copy($zeroOffset, 0x200, $offsetZeroBytes, 0, $offsetZeroBytes.Length)
    Save-Binary $p1b $offsetZeroBytes
    Assert-Case -Name 'C1b: SRAM valida offset 0' -SramPath $p1b -Expected 'pass'
    ...
    # Case 8: output contem frame_metrics sinteticos a partir das samples
    $script:cases++
    $p8 = Join-Path $tempRoot 'c8_frame_metrics.sram'
    $p8Samples = @(41, 55, 64)
    $words8 = [Math]::Max(64, 32 + $p8Samples.Count)
    Save-Binary $p8 (New-SyntheticMdrt -WordCount $words8 -SamplesRecorded $p8Samples.Count -SceneId 2 -SampleValues $p8Samples)
    $p8Out = Join-Path $tempRoot 'c8_frame_metrics.json'
    & $parser -SramPath $p8 -OutputPath $p8Out -SramOffset 0x200 -FrameWindow 1800 | Out-Null
    $p8Json = Get-Content -LiteralPath $p8Out -Raw -Encoding UTF8 | ConvertFrom-Json
    $p8Ok = (
        $p8Json.frame_metrics_kind -eq 'mdrt_sample_series' -and
        @($p8Json.frame_metrics).Count -eq 3 -and
        $p8Json.frame_metrics[0].scene_id -eq 2 -and
        $p8Json.frame_metrics[0].cpu_load_ratio -eq 41 -and
        $p8Json.frame_metrics[2].cpu_load_ratio -eq 64
    )
    if ($p8Ok) {
