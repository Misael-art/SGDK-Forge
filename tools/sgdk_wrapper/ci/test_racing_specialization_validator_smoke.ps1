<#
.SYNOPSIS
    Smoke test for validate_racing_arcade_specialization.ps1 in generalista path.
#>
...
$rogueRoot = Join-Path $workspaceRoot 'out\ci\racing_validator_smoke_fixture'
$reportPath = Join-Path $rogueRoot 'out\logs\racing_specialization_report.json'
...
Write-Host ''
Write-Host '=== Racing Specialization Validator Smoke Test ==='
Write-Host ''
...
if (Test-Path -LiteralPath $reportPath) {
    ...
    Assert-True 'report has 0 vehicle_audits' ($report.vehicle_audits.Count -eq 0) ("$($report.vehicle_audits.Count) audits")
