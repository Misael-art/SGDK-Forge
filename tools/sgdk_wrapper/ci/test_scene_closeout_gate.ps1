...
Assert-True ($report.status -eq "planned") "Expected planned status, got '$($report.status)'"
...
Assert-True ($blockedReport.status -eq "blocked") "Expected blocked status, got '$($blockedReport.status)'"
