...
if ($report) {
    ...
    $unsupCanonical = $sceneRows | Where-Object { $_.scene_key -eq 'unsupported_scene' } | Select-Object -First 1
    Assert-True 'Unsupported canonical row status=failed' ($unsupCanonical -and $unsupCanonical.status -eq 'failed')
    ...
    $directCanonical = $sceneRows | Where-Object { $_.scene_key -eq 'direct_scene' } | Select-Object -First 1
    Assert-True 'Direct canonical row status=failed' ($directCanonical -and $directCanonical.status -eq 'failed')
