<#
.SYNOPSIS
    Smoke test for validate_strategy_tower_defense_specialization.ps1 in
...
$rogueRoot = Join-Path $workspaceRoot 'out\ci\strategy_validator_smoke_fixture'
$reportPath = Join-Path $rogueRoot 'out\logs\strategy_specialization_report.json'
...
Write-Host ''
Write-Host '=== Strategy Specialization Validator Smoke Test ==='
Write-Host ''
...
if (Test-Path -LiteralPath $reportPath) {
    ...
    Assert-True 'report has 0 tower_audits' ($report.tower_audits.Count -eq 0) ("$($report.tower_audits.Count) audits")
