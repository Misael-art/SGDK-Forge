...
- `<project_root>/out/evidence/blastem/visual_vdp_dump.bin` quando existir exportador legado compativel
- `<project_root>/out/evidence/blastem/session_manifest.json`

Contrato canonico atual:

- `screenshot.png` + `save.sram` formam o bundle minimo canonico
- `save.sram` e a evidencia tecnica principal quando contem heartbeat e bloco `MDRT`
- `visual_vdp_dump.bin` permanece como recurso historico legado, usado anteriormente em fluxos baseados em `VLAB`
...
Invoke-BlastemEvidenceCapture -Session <hashtable> -CaptureMode <string> -EvidenceRoot <string>
Test-BlastemEvidenceBundle -SessionRoot <string> -RequireVdpDump
```

Observacao:

- `-RequireVdpDump` so deve ser usado por pipelines legados que ainda dependem de `visual_vdp_dump.bin`; nao e o padrao recomendado para a V2 baseada em `MDRT`
...
- `required_artifacts` controla apenas a presenca minima da captura.
- `comparison_artifacts` controla exclusivamente o diff de baseline.
