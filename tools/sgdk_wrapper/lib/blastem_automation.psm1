...
$script:BlastEmSdlHwndByPid = @{}
$script:_BlastEmEnumResults = @()
$script:_BlastEmEnumTargetPid = [uint32]0
...
function Get-BlastEmSdlWindowClassName {
    param(
    ...
    [void][SGDKBlastEmWin32]::GetClassNameW($Hwnd, $buffer, 256)
    return $buffer.ToString()
}

function Get-BlastEmSdlWindowInternal {
    <#
    Resolve o HWND da janela SDL visível do processo BlastEm. SDL2 cria uma janela
    com class_name "SDL_app"; é para esse HWND que o input injetado por SendInput
    precisa ter foreground. Process.MainWindowHandle nao e garantido ser essa janela.

    Estrategia:
      1. Checar cache por PID (valido enquanto IsWindow retornar true).
      2. Enumerar janelas top-level, filtrar por PID + IsWindowVisible + class SDL_app.
      3. Fallback: Process.MainWindowHandle (com warning no caller via log).
    #>
    param(
    ...
    if (-not $script:BlastEmSdlHwndByPid) {
        $script:BlastEmSdlHwndByPid = @{}
    }

    if ($script:BlastEmSdlHwndByPid.ContainsKey($targetPid)) {
        $cached = [IntPtr]$script:BlastEmSdlHwndByPid[$targetPid]
        if ($cached -ne [IntPtr]::Zero -and [SGDKBlastEmWin32]::IsWindow($cached)) {
            return $cached
        }
        [void]$script:BlastEmSdlHwndByPid.Remove($targetPid)
    }

    $script:_BlastEmEnumTargetPid = $targetPid
    $script:_BlastEmEnumResults = [System.Collections.ArrayList]::new()

    $callback = [SGDKBlastEmWin32+EnumWindowsProc]{
        param([IntPtr]$hWnd, [IntPtr]$lParam)
        $wpid = [uint32]0
        [void][SGDKBlastEmWin32]::GetWindowThreadProcessId($hWnd, [ref]$wpid)
        if ($wpid -eq $script:_BlastEmEnumTargetPid) {
            if ([SGDKBlastEmWin32]::IsWindowVisible($hWnd)) {
                $sb = New-Object System.Text.StringBuilder 256
                [void][SGDKBlastEmWin32]::GetClassNameW($hWnd, $sb, 256)
                [void]$script:_BlastEmEnumResults.Add(@{
                    Hwnd  = $hWnd
                    Class = $sb.ToString()
                })
            }
    ...
    try {
        [void][SGDKBlastEmWin32]::EnumWindows($callback, [IntPtr]::Zero)
    }
    finally {
        $script:_BlastEmEnumTargetPid = [uint32]0
    }

    $results = @($script:_BlastEmEnumResults)
    $script:_BlastEmEnumResults = $null

    $sdlWindow = $results | Where-Object { $_.Class -eq 'SDL_app' } | Select-Object -First 1
    if ($sdlWindow) {
        $script:BlastEmSdlHwndByPid[$targetPid] = [IntPtr]$sdlWindow.Hwnd
        return [IntPtr]$sdlWindow.Hwnd
    }

    $Process.Refresh()
    if ($Process.MainWindowHandle -ne [IntPtr]::Zero) {
        $script:BlastEmSdlHwndByPid[$targetPid] = [IntPtr]$Process.MainWindowHandle
...
function Ensure-BlastEmForeground {
    ...
    $Process.Refresh()
    $hwnd = Get-BlastEmSdlWindowInternal -Process $Process
    if ($hwnd -eq [IntPtr]::Zero) {
        throw "BlastEm nao expos janela SDL (SDL_app) para navegacao automatica."
    }

    for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
        [SGDKBlastEmWin32]::ShowWindow($hwnd, 9) | Out-Null
        [SGDKBlastEmWin32]::BringWindowToTop($hwnd) | Out-Null
        [SGDKBlastEmWin32]::SetWindowPos($hwnd, [IntPtr]::new(-1), 40, 40, 640, 480, 0x0040) | Out-Null
        ...
        if ([SGDKBlastEmWin32]::GetForegroundWindow() -ne $hwnd) {
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace($Process.MainWindowTitle)) {
                    try {
                        ...
                        # Keep trying with the native Win32 path below.
                    }
                }
            }
        }
...
function Send-BlastEmKey {
    <#
    Contrato canonico de input para BlastEm (SDL2):
      - Unica estrategia: SendInput com KEYEVENTF_SCANCODE (+ EXTENDEDKEY quando
        aplicavel). SDL2 deriva SDL_Scancode do scan code em WM_INPUT/WM_KEYDOWN;
        qualquer outro caminho (SendKeys VK-only, PostMessage lParam=0) e frágil
        para apps SDL.
      - Antes de injetar, exige foreground no HWND SDL (class_name=SDL_app).
        Se foreground nao for conquistado apos MaxForegroundAttempts, lanca erro
        determinista e o caller decide (teardown/retry de sessao). Sem fallback
        silencioso.
    #>
    param(
        [Parameter(Mandatory = $true)][System.Diagnostics.Process]$Process,
        ...
        [Parameter(Mandatory = $false)][string]$LogPath = "",
        [Parameter(Mandatory = $false)][int]$MaxForegroundAttempts = 6
    )
    ...
    $foregroundOk = Ensure-BlastEmForeground -Process $Process -MaxAttempts $MaxForegroundAttempts
    if (-not $foregroundOk) {
        throw ("BlastEm nao pode receber input: foreground indisponivel apos " + $MaxForegroundAttempts + " tentativas (vk=0x{0:X})." -f [int]$VirtualKey)
    }

    $sdlHwnd = Get-BlastEmSdlWindowInternal -Process $Process
    $className = Get-BlastEmSdlWindowClassName -Hwnd $sdlHwnd
    $fgHwnd = [SGDKBlastEmWin32]::GetForegroundWindow()
    $fgPid = [uint32]0
    if ($fgHwnd -ne [IntPtr]::Zero) {
        [void][SGDKBlastEmWin32]::GetWindowThreadProcessId($fgHwnd, [ref]$fgPid)
    }

    if (-not [string]::IsNullOrWhiteSpace($LogPath)) {
        Write-BlastEmCaptureLog -LogPath $LogPath -Event "input_dispatch" -Data @{
            delivery        = "sendinput"
            foreground_ok   = [bool]$foregroundOk
            virtual_key     = ("0x{0:X}" -f $VirtualKey)
            extended        = [bool]$Extended
            hold_ms         = [int]$HoldMilliseconds
            hwnd            = ("0x{0:X}" -f [int64]$sdlHwnd)
            class_name      = $className
            foreground_hwnd = ("0x{0:X}" -f [int64]$fgHwnd)
            foreground_pid  = [int64]$fgPid
        }
    }
...
function Save-BlastEmWindowScreenshot {
    ...
    try {
        ...
        $targetHwnd = Get-BlastEmSdlWindowInternal -Process $Process
        if ($targetHwnd -eq [IntPtr]::Zero) { return $false }

        $rect = New-Object "SGDKBlastEmWin32+RECT"
        if (-not [SGDKBlastEmWin32]::GetWindowRect($targetHwnd, [ref]$rect)) {
            return $false
        }
        ...
        $desiredWidth = [Math]::Max($width, 640)
        $desiredHeight = [Math]::Max($height, 480)

        [SGDKBlastEmWin32]::ShowWindow($targetHwnd, 9) | Out-Null
        [SGDKBlastEmWin32]::SetWindowPos($targetHwnd, [IntPtr]::new(-1), 40, 40, $desiredWidth, $desiredHeight, 0x0040) | Out-Null
        [SGDKBlastEmWin32]::BringWindowToTop($targetHwnd) | Out-Null
        [SGDKBlastEmWin32]::SetForegroundWindow($targetHwnd) | Out-Null
        Start-Sleep -Milliseconds 250

        $visibleRect = New-Object "SGDKBlastEmWin32+RECT"
        if ([SGDKBlastEmWin32]::GetWindowRect($targetHwnd, [ref]$visibleRect)) {
            $rect = $visibleRect
            $width = $rect.Right - $rect.Left
            $height = $rect.Bottom - $rect.Top
            if ($width -le 0 -or $height -le 0) {
        ...
        try {
            ...
            try {
                ...
                $printed = $false
                try {
                    $printed = [SGDKBlastEmWin32]::PrintWindow($targetHwnd, $hdc, 0)
                }
                finally {
                    $graphics.ReleaseHdc($hdc)
                }

                $blankPrintWindow = $false
                if ($printed) {
                    $sampleCount = 0
                    $whiteLikeCount = 0
                    $blackLikeCount = 0
                    $stepX = [Math]::Max(1, [int]($width / 12))
                    $stepY = [Math]::Max(1, [int]($height / 9))
                    ...
                    $blankPrintWindow = (
                        $sampleCount -gt 0 -and (
                            ($whiteLikeCount / $sampleCount) -gt 0.96 -or
                            ($blackLikeCount / $sampleCount) -gt 0.96
                        )
                    )
                }

                if (-not $printed -or $blankPrintWindow) {
                    [SGDKBlastEmWin32]::SetWindowPos($targetHwnd, [IntPtr]::new(-1), 40, 40, $width, $height, 0x0040) | Out-Null
                    [SGDKBlastEmWin32]::BringWindowToTop($targetHwnd) | Out-Null
                    [SGDKBlastEmWin32]::SetForegroundWindow($targetHwnd) | Out-Null
                    Start-Sleep -Milliseconds 250
                    $copyRect = New-Object "SGDKBlastEmWin32+RECT"
                    if ([SGDKBlastEmWin32]::GetWindowRect($targetHwnd, [ref]$copyRect)) {
                        $rect = $copyRect
                    }
...
function Write-BlastEmConfig {
    ...
    [System.IO.File]::WriteAllText($TargetConfigPath, $finalContent, [System.Text.Encoding]::UTF8)
}
...
function Close-BlastEmGracefully {
    ...
    try {
        Send-BlastEmKey -Process $Process -VirtualKey 0x1B -Extended $false -HoldMilliseconds 90 -LogPath $LogPath
...
function Test-MDRTRuntimeSignature {
    ...
    try {
        ...
        [void]$offsetCandidates.Add($Offset)
        if ($Offset -eq 0x200) {
            # Some projects materialize MDRT at SRAM base instead of +0x200.
            [void]$offsetCandidates.Add(0)
        }

        foreach ($candidateOffset in ($offsetCandidates | Select-Object -Unique)) {
            if ($bytes.Length -lt ($candidateOffset + 4)) {
                continue
            }

            if ([System.Text.Encoding]::ASCII.GetString($bytes, $candidateOffset, 4) -eq "MDRT") {
                return $true
            }
        }
...
function Invoke-BlastEmFlushCycle {
    ...
    #>
    try {
        Send-BlastEmKey -Process $Process -VirtualKey 0x1B -Extended $false -HoldMilliseconds 60 -LogPath $LogPath
    ...
    foreach ($rawStep in $Sequence) {
        ...
        if ($step.StartsWith("mash:")) {
            ...
            for ($i = 0; $i -lt $count; $i++) {
                ...
                Send-BlastEmKey -Process $Process -VirtualKey $key.vk -Extended $key.ext -HoldMilliseconds $hold -LogPath $LogPath
        ...
        if ($step.StartsWith("hold:")) {
            ...
            Send-BlastEmKey -Process $Process -VirtualKey $key.vk -Extended $key.ext -HoldMilliseconds $duration -LogPath $LogPath
        ...
        if ($step -eq "region_unlock") {
            ...
            for ($i = 0; $i -lt 3; $i++) {
                if ($Process.HasExited) { break }
                Send-BlastEmKey -Process $Process -VirtualKey $key.vk -Extended $key.ext -HoldMilliseconds 80 -LogPath $LogPath
            ...
            try {
                foreach ($attemptKeyName in $attempts) {
                    ...
                    while ([datetime]::UtcNow -lt $attemptDeadlineUtc -and $attemptPresses -lt $maxPresses) {
                        ...
                        Send-BlastEmKey -Process $Process -VirtualKey $attemptKey.vk -Extended $attemptKey.ext -HoldMilliseconds $hold -LogPath $LogPath
                        $attemptPresses++
                        $totalPresses++
...
Export-ModuleMember -Function @(
    ...
    'Start-BlastEmSaveWatchers',
    'Stop-BlastEmSaveWatchers',
    'Wait-ForSramChangeOrDeadline',
