...
function Invoke-ProjectArtifactChecks {
    param([string]$Root)
    ...
    $requiredArtifacts = @(
        ...
        "doc\\technique_usage_manifest.json",
        "rascunho\\README.md",
    ...
    foreach ($relativePath in $requiredArtifacts) {
        ...
        if (-not (Test-Path -LiteralPath $fullPath -PathType Leaf)) {
            Write-PreflightLog "WARN" ("artefato minimo ausente: {0}" -f $relativePath)
            $script:softWarnings++
        }
    }
}
...
try {
    ...
    if (-not (Test-Path -LiteralPath $makefileGen -PathType Leaf)) {
        ...
        $localGdk = [System.IO.Path]::GetFullPath((Join-Path $mdRoot "sdk\sgdk-2.11"))
        if (-not $gdk.Equals($localGdk, [System.StringComparison]::OrdinalIgnoreCase)) {
            Write-PreflightLog "ERROR" "GDK externo nao e canonico para build/closeout. Materialize sdk\sgdk-2.11 no workspace: $gdk"
            $script:blockingFailures++
        }
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($ProjectRoot)) {
        ...
        Invoke-ProjectArtifactChecks -Root $ProjectRoot
    }
