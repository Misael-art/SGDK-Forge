    ...
    if ($StatusOnly) {
        if ($latest) {
            $currentAssetVersions += [pscustomobject]@{
                ...
                source_sha256 = $latest.meta.source_sha256
...
if ($romIdentity) {
    ...
    } elseif ($latestBuild -and $latestBuild.meta.rom_sha256 -eq $romIdentity.sha256) {
        $buildVersion = $latestBuild.version
        $buildMetaPath = $latestBuild.meta_path
    } else {
    ...
    if (-not $StatusOnly) {
        ...
        $buildMeta = [ordered]@{
            ...
            status_panel_snapshot = if ($validationReport) {
                [ordered]@{
                    visual_lab_aprovado = [bool]$validationReport.status_panel.visual_lab_aprovado
                    gameplay_rom_aprovada = [bool]$validationReport.status_panel.gameplay_rom_aprovada
                    ready_for_aaa = [bool]$validationReport.status_panel.ready_for_aaa
                }
            } else {
                $null
            }
            qa_axes_snapshot = if ($validationReport) {
                [ordered]@{
                    gameplay_basico = [string]$validationReport.qa_axes.gameplay_basico
                    performance = [string]$validationReport.qa_axes.performance
                    audio = [string]$validationReport.qa_axes.audio
                    hardware_real = [string]$validationReport.qa_axes.hardware_real
                }
            } else {
                $null
            }
...
if (-not $StatusOnly -and -not (Test-Path -LiteralPath $changelogPath -PathType Leaf)) {
    @(
        "# Changelog Canonico"
        ""
...
$hasExplicitTaskContext = (-not [string]::IsNullOrWhiteSpace($Notes)) -or ($Task -ne "wrapper_snapshot") -or ($Skills.Count -gt 0)
$shouldAppendEntry = (-not $StatusOnly) -and ($createdRomSnapshot -or ($newAssetVersions.Count -gt 0) -or $hasExplicitTaskContext)
...
if ($validationReport) {
    $generatedStatus += ('- Gate visual: visual_lab_aprovado={0}' -f ([bool]$validationReport.status_panel.visual_lab_aprovado))
    $generatedStatus += ('- Gate gameplay: gameplay_rom_aprovada={0}' -f ([bool]$validationReport.status_panel.gameplay_rom_aprovada))
    $generatedStatus += ('- Gate AAA: ready_for_aaa={0}' -f ([bool]$validationReport.status_panel.ready_for_aaa))
    $generatedStatus += ('- QA runtime: gameplay={0} performance={1} audio={2} hardware_real={3}' -f
        [string]$validationReport.qa_axes.gameplay_basico,
        [string]$validationReport.qa_axes.performance,
        [string]$validationReport.qa_axes.audio,
        [string]$validationReport.qa_axes.hardware_real)
}
