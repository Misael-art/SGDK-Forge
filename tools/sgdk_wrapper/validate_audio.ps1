...
function Write-Log {
    ...
    [System.IO.File]::AppendAllText($DEBUG_LOG, $fullMessage + [Environment]::NewLine, [System.Text.Encoding]::UTF8)
}
    ...
    foreach ($resFile in $resFiles) {
        ...
        foreach ($line in (Get-Content -LiteralPath $resFile.FullName -ErrorAction SilentlyContinue)) {
            ...
            $declarations += [pscustomobject]@{
                ...
                resolved_relative_path = $(if ($exists) { Get-ProjectRelativePath $resolvedPath } else { Get-ProjectRelativePath $resolvedPath })
...
function Get-AudioDeclarationsShared {
    $resFiles = @(Get-SgdkResFiles -ProjectRoot $ProjectRoot)
    ...
    $parsedDeclarations = @(Get-SgdkResDeclarations -ResFiles $resFiles -ProjectRoot $ProjectRoot)
    foreach ($decl in $parsedDeclarations) {
        if ($decl.parser_status -eq "unparsed") {
            continue
        }
        if ($decl.classification -ne "audio") {
            continue
        }
        if ($decl.resource_kind -eq "BIN_AUDIO" -and -not (Test-SupportedAudioBinDeclaration -DeclaredPath $decl.declared_path)) {
            continue
        }

        $exists = [bool]$decl.exists
        if ($exists -and -not [string]::IsNullOrWhiteSpace([string]$decl.resolved_path)) {
            $script:LatestSourceWriteUtc = Update-LatestTimestamp -Current $script:LatestSourceWriteUtc -CandidatePath $decl.resolved_path
        }

        $driver = [string]$decl.resource_kind
        $declaredRate = $null
        if ($decl.resource_kind -eq "WAV") {
            $driver = "UNKNOWN"
            if ($decl.normalized_options -and $decl.normalized_options.Contains("driver")) {
                $driver = [string]$decl.normalized_options["driver"]
            }
            if ($decl.normalized_options -and $decl.normalized_options.Contains("rate")) {
                $declaredRate = [int]$decl.normalized_options["rate"]
            }
        } elseif ($decl.resource_kind -eq "BIN_AUDIO") {
            $driver = "BIN_AUDIO"
        }

        $resolvedRelativePath = $null
        if ($exists -and -not [string]::IsNullOrWhiteSpace([string]$decl.resolved_path)) {
            $resolvedRelativePath = Get-ProjectRelativePath $decl.resolved_path
        } elseif (-not [string]::IsNullOrWhiteSpace([string]$decl.resolved_path)) {
            $resolvedRelativePath = Get-ProjectRelativePath $decl.resolved_path
        } else {
            $resolvedRelativePath = [string]$decl.declared_path
        }

        $sourceExtensionPath = if (-not [string]::IsNullOrWhiteSpace([string]$decl.resolved_path)) {
            [string]$decl.resolved_path
        } else {
            [string]$decl.declared_path
        }

        $declarations += [pscustomobject]@{
            resource_kind = $decl.resource_kind
            resource_name = $decl.resource_name
            source_res_file = $decl.res_file
            source_res_path = Get-ProjectRelativePath $decl.res_file
            source_res_line = $decl.res_line
            declared_path = $decl.declared_path
            resolved_path = $decl.resolved_path
            resolved_relative_path = $resolvedRelativePath
            driver = $driver
            declared_rate = $declaredRate
            source_exists = $exists
            source_extension = [System.IO.Path]::GetExtension($sourceExtensionPath).ToLowerInvariant()
            source_size_bytes = $(if ($decl.source_size_bytes) { [int64]$decl.source_size_bytes } else { 0 })
        }
    }
...
$audioDeclarations = @(Get-AudioDeclarationsShared)
